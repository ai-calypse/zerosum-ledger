...
|---|---|---|---|---|
| S00-T01 | Done | `LICENSE`, `.gitignore`, `README.md`, `docs/adr/0001-record-architecture-decisions.md`; first commit `d65384c` contains only `docs/` with hashes matching I.2 | `git log --reverse --stat`; `shasum -a 256 -c` 0 mismatches; `git check-ignore`; pushed to `https://github.com/ai-calypse/zerosum-ledger` on 2026-09-15 | — |
| S00-T02 | Done (revalidated after CR-S02-05) | `settings.gradle.kts`, `gradle/`, `gradlew*`, `build-logic/`, `libs/*`, `services/*`, `tools/*` (`09cedee`) | H.4 rows 1–3 | — |
...
| S00-T05 | Done | `.github/workflows/ci.yml` (`eeb4272`, `f840db0`) | Publishing grep and YAML parse passed; dispatched run green (build, integration, e2e), clean push-triggered run green, deliberate-failure run red — all in [docs/results/s00/s00-t05-ci-runs.txt](results/s00/s00-t05-ci-runs.txt) | — |
