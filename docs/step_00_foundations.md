...
| CI build and integration jobs | GitHub Actions run on `main` (S00-T05) | **Passed** on [run 35037237318](https://github.com/ai-calypse/zerosum-ledger/actions/runs/35037237318) (`workflow_dispatch` on `main` at `2f5dbcf`): `build + unit` success in **2 m 05 s**, `integration (Testcontainers)` success in **1 m 08 s**, whole run 5 m 22 s — within the ≤ 10 min push target and the ≤ 10 min S00 exit criterion. The push-triggered run [35037158088](https://github.com/ai-calypse/zerosum-ledger/actions/runs/35037158088) was **cancelled** by this workflow's own `cancel-in-progress` concurrency group when the dispatch superseded it; a clean push-triggered run is recorded when S02 merges | [docs/results/s00/s00-t05-ci-runs.txt](results/s00/s00-t05-ci-runs.txt) | 2026-09-15 |
...
| CI detects failure | Deliberately failing test on a throwaway branch (S00-T05) | **Not run**: scheduled once S02-T02 is committed, so the throwaway branch starts from a clean tree | — | — |
...
| **GitHub remote and CI runs** (S00-T01 push; S00-T05 green `build`/`integration` on `main`, `gh workflow run` e2e, failing-test run) | **resolved 2026-09-15** | The builder authorized the public repository `ai-calypse/zerosum-ledger`; `main` and the step branches are pushed, and `build`, `integration` and `e2e` are green (H.4). Remaining: one clean push-triggered run (the first was cancelled by the concurrency group) and the deliberate-failure run, both scheduled after S02-T02 | S00-T05 records the two outstanding runs in H.4; nothing downstream is blocked |
