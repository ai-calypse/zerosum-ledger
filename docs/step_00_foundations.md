...
| D00-3 | **Compose topology:** default profile contents; the `chaos` profile adding toxiproxy; memory limits; PostgreSQL server configuration, including the `pg_stat_statements` preload (master §0.3 O12); health checks and start periods; ports and host binding; volumes; startup ordering; stop grace period; restart policy; service image build method; broker-level Kafka settings such as auto topic creation and internal-topic replication for a single broker. Also the **extension rule**: later steps add containers or settings in their own register with trace comments, while changes to existing D00-3 choices use a change request. | [docs/zerosum_ledger_mvp_plan.md#topology](zerosum_ledger_mvp_plan.md#topology), [docs/zerosum_ledger_mvp_plan.md#environments](zerosum_ledger_mvp_plan.md#environments), [docs/zerosum_ledger_mvp_plan.md#cold-warm](zerosum_ledger_mvp_plan.md#cold-warm), [docs/zerosum_ledger_mvp_plan.md#instrumentation](zerosum_ledger_mvp_plan.md#instrumentation), [docs/zerosum_ledger_mvp_plan.md#decomposition-clarifications](zerosum_ledger_mvp_plan.md#decomposition-clarifications) (O12) **Amended by CR-S07-01 (S07, 2026-09-16):** `otel-lgtm` mounts `infra/grafana/provisioning/{dashboards,alerting}` read-only, so D07-2 dashboards and D07-3 alert rules are provisioned from the repository rather than imported by hand. No new ports, images or network exposure |
...
- [x] **CI:** the `build` and `integration` jobs are green on `main`, and their duration is recorded against the exit criterion in [docs/zerosum_ledger_mvp_plan.md#step-00](zerosum_ledger_mvp_plan.md#step-00) and the push target in [docs/zerosum_ledger_mvp_plan.md#cicd](zerosum_ledger_mvp_plan.md#cicd). The `e2e` job skeleton has run green at least once. *(Dispatched and push-triggered runs green; durations in H.4.)*
...
| D00-2 | Gradle projects: `:libs:{money,contracts,outbox,auth}`, `:services:{order-service,ledger-service,instrument-service,fake-providers}`, `:tools:{simulator,verifier}`, `:infra:tests` (test-only project for `DatabaseIsolationIT`). `:libs`, `:services`, `:tools`, `:infra` are empty container projects created by the hierarchical includes. Convention plugins live in the included build `build-logic/`. `tools/chaos`, `tools/k6` are not Gradle projects (created by S07/S08). Helper script: `tools/dev/generate-env.sh`. Base package `dev.zerosum.<module>`. Spike `spikes/sp3-stack` existed only at `e87c4e7`. Git: local repository, `main` plus one branch per step (`step/NN-name`, merged `--no-ff`); **no GitHub remote yet** (H.5). | Mirrors master §5.1; infrastructure tests kept out of service code | `buildSrc/` (less explicit, invalidates the whole build on change); flat project names (less readable) | Accepted | 2026-09-15 |
...
|---|---|---|---|
| License | `LICENSE` | `LICENSE` (MIT) | `9c1b922` |
| Git ignore rules | `.gitignore` | `.gitignore` | `9c1b922` |
| Root README (stub, then quickstart skeleton) | `README.md` | `README.md` | `2d762a3` |
| ADR-0001 | `docs/adr/0001-record-architecture-decisions.md` | same | `9c1b922` |
| ADR-0002 | `docs/adr/0002-stack-and-pinned-versions.md` | same (Accepted) | `2d762a3` |
| Results template | `docs/results/TEMPLATE.md` | same | `04006a3` |
| SP3 results | `docs/results/sp3-stack-compat.md` | same, raw data in `docs/results/sp3-raw/` | `1109233` |
| SP3 spike code (removed after decision) | `spikes/sp3-stack/` (commit SHA only) | `spikes/sp3-stack/` at `e87c4e7`; removed in `11d99ff` | `e87c4e7` |
| Cross-database isolation test | `DatabaseIsolationIT` at the D00-2 location | `infra/tests/src/test/java/dev/zerosum/infra/DatabaseIsolationIT.java` | `6cb70b8` |
| Secrets and log-redaction note | `docs/secrets.md` | same | `2d762a3` |
...
| CI build and integration jobs | GitHub Actions run on `main` (S00-T05) | **Passed** on [run 35037237318](https://github.com/ai-calypse/zerosum-ledger/actions/runs/35037237318) (`workflow_dispatch` on `main` at `2a5233a`): `build + unit` success in **2 m 05 s**, `integration (Testcontainers)` success in **1 m 08 s**, whole run 5 m 22 s — within the ≤ 10 min push target and the ≤ 10 min S00 exit criterion. The first push-triggered run [35037158088](https://github.com/ai-calypse/zerosum-ledger/actions/runs/35037158088) was **cancelled** by this workflow’s own `cancel-in-progress` concurrency group when the dispatch superseded it. A clean push-triggered run followed on `step/02-ledger-core`: [35038096697](https://github.com/ai-calypse/zerosum-ledger/actions/runs/35038096697) **success**, `build + unit` 1 m 10 s, `integration` 1 m 43 s, run total 3 m 01 s, with `e2e` correctly skipped outside schedule and dispatch | [docs/results/s00/s00-t05-ci-runs.txt](results/s00/s00-t05-ci-runs.txt) | 2026-09-15 |
...
| CI e2e skeleton | `gh workflow run ci.yml` (S00-T05) | **Passed**: the dispatched run's `e2e (Compose)` job succeeded in **2 m 52 s** (generate masked `.env`, `./gradlew assemble`, `docker compose up -d --build --wait` healthy, `e2eTest` with no tests selected yet, `down -v`) — within the ≤ 20 min nightly target | [docs/results/s00/s00-t05-ci-runs.txt](results/s00/s00-t05-ci-runs.txt) | 2026-09-15 |
| CI detects failure | Deliberately failing test on a throwaway branch (S00-T05) | **Passed**: branch `ci-failure-probe` with one failing unit test → [run 35038352739](https://github.com/ai-calypse/zerosum-ledger/actions/runs/35038352739) conclusion **failure** (`build + unit` failed; `integration` and `e2e` skipped). Branch deleted locally and remotely afterwards | [docs/results/s00/s00-t05-ci-runs.txt](results/s00/s00-t05-ci-runs.txt) | 2026-09-15 |
| ADR-0002 matches artifacts | Comparison of catalog, `image:` lines, `FROM`, `uses:` and wrapper against ADR-0002 (S00-T06) | **Passed**: scripted `grep -F` of every catalog `[versions]` value, Compose `image:`, Dockerfile `FROM`, CI `uses:` SHA, wrapper version/checksum and foojay version against ADR-0002 → 18 checked, **0 mismatches**; 0 untagged or `latest` images | [docs/results/s00/s00-t06-adr-vs-artifacts.txt](results/s00/s00-t06-adr-vs-artifacts.txt) (artifacts at `2d762a3`) | 2026-09-15 |
...
| **GitHub remote and CI runs** (S00-T01 push; S00-T05 CI evidence) | **resolved 2026-09-15** | The builder authorized the public repository `ai-calypse/zerosum-ledger`. `main` and the step branches are pushed; `build`, `integration`, `e2e`, a clean push-triggered run and the deliberate-failure run are all recorded in H.4 | None. Nothing remains blocked |
...
| Field | Value |
|---|---|
| Step status | **Done**. The GitHub-dependent evidence was completed on 2026-09-15 after the builder authorized the public repository. |
...
|---|---|---|---|---|
| S00-T01 | Done | `LICENSE`, `.gitignore`, `README.md`, `docs/adr/0001-record-architecture-decisions.md`; first commit `1df78b3` contains only `docs/` with hashes matching I.2 | `git log --reverse --stat`; `shasum -a 256 -c` 0 mismatches; `git check-ignore`; pushed to `https://github.com/ai-calypse/zerosum-ledger` on 2026-09-15 | — |
| S00-T02 | Done (revalidated after CR-S02-05) | `settings.gradle.kts`, `gradle/`, `gradlew*`, `build-logic/`, `libs/*`, `services/*`, `tools/*` (`0130cde`) | H.4 rows 1–3 | — |
| S00-T03 | Done | `docker-compose.yml` (`6cb70b8`) | H.4 rows 4–8 | Lost about 0.5 h to the Docker Desktop wedge (H.5) |
| S00-T04 | Done | `infra/postgres/{init.sh,init.sql,database.sql}`, `V1__baseline.sql` ×4, service `application.yml`, `infra/tests/…/DatabaseIsolationIT.java` (`6cb70b8`) | H.4 rows 9–12 | — |
| S00-T05 | Done | `.github/workflows/ci.yml` (`45b8972`, `2d762a3`) | Publishing grep and YAML parse passed; dispatched run green (build, integration, e2e), clean push-triggered run green, deliberate-failure run red — all in [docs/results/s00/s00-t05-ci-runs.txt](results/s00/s00-t05-ci-runs.txt) | — |
| S00-T06 | Done | `docs/adr/0002-stack-and-pinned-versions.md`, `docs/results/TEMPLATE.md`, `README.md` (`04006a3`) | H.4 "ADR-0002 matches artifacts" | — |
| S00-T07 | Done | `docs/results/sp3-stack-compat.md`, `docs/results/sp3-raw/`, ADR-0002 Accepted (`1109233`); spike at `e87c4e7`, removed in `11d99ff` | Started ~21:30Z, G0 decided 21:50Z (≈0.4 h of the 2 h timebox; no O1 contingency used) | — |
| S00-T08 | Done | `services/Dockerfile`, service entries in `docker-compose.yml`, observability sections of `application.yml`, `zs.service-conventions` (`2d762a3`) | H.4 rows 18–21 | — |
| S00-T09 | Done | `.env.example`, `tools/dev/generate-env.sh`, `docs/secrets.md`, CI e2e job and README use the script (`6cb70b8`, `2d762a3`) | H.4 rows 22–24 | — |
...
|---|---|---|---|---|
| `docs/zerosum_ledger_mvp_plan.md` | sha256 `2ec6499c…c342c` before git; then `1df78b3 2026-09-15` | 2026-09-15 (start; rechecked at every phase boundary and before handoff) | S00-T01 to S00-T09 | No impact: unchanged since `1df78b3` (`git diff --stat 1df78b3..HEAD` empty) |
| `docs/README.md` | sha256 `f33c18cf…606cfa` before git; then `1df78b3 2026-09-15` | 2026-09-15 (same checkpoints) | S00-T01 to S00-T09 | No impact: unchanged since `1df78b3` |
| `docs/zerosum_ledger_mvp_plan.md` v1.2 §0.3 (O1, O2, O3, O4, O12, C9, C16) | `1df78b3` | 2026-09-15 (doc version 1.1) | S00-T02, S00-T03, S00-T04, S00-T05, S00-T06, S00-T07 | Resolved in master v1.2 (§0.3 O1, O2, O3, O4, O12, C9, C16); instructions and verification updated |
| `docs/step_00_foundations.md` | sha256 `644b04c1…3f701` before git; `1df78b3` at import; registers updated in `dad664d` and this commit | 2026-09-15 | S00-T01 to S00-T09 | Register-only edits by this step (sections H, I); task text unchanged |
...
| `docs/adr/0002-stack-and-pinned-versions.md` (consumed by later S00 tasks once written) | `04006a3` (Proposed) → `1109233` (Accepted) → `2d762a3` (+ Temurin base image row) | 2026-09-15 | S00-T07, S00-T08 | Revalidated: T08 added a pin under the D00-1 policy; no decision changed |
| `docker-compose.yml` (consumed by later S00 tasks once written) | `6cb70b8` (infrastructure) → `2d762a3` (+ services) | 2026-09-15 | S00-T04, S00-T05, S00-T07, S00-T08, S00-T09 | Revalidated: T08 appended service entries without changing any existing D00-3 choice; `DatabaseIsolationIT` still reads the same postgres pin |
| `gradle/libs.versions.toml` | `0130cde` → `6cb70b8` → `e87c4e7` | 2026-09-15 | S00-T04, S00-T07, S00-T08 | Revalidated: additions only (JDBC/Flyway/Testcontainers, Kafka/OTLP entries kept for S04/T08); no pinned version changed |
