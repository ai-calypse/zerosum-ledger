...
| Webhook receiver | `services/instrument-service/src/main/java/` | `services/instrument-service/src/main/java/dev/zerosum/instrument/webhooks/` (`WebhookReceiver`, `AheadOfStateResolver`), `.../instrument/api/WebhookController.java` (the endpoint, beside the other controllers so one exception handler covers them all) and `.../instrument/adapter/ProviderWebhooks.java` (verification and normalization, inside the adapter boundary) | D05-3, D05-5 |
...
|---|---|---|---|
| Instrument-service OpenAPI | `openapi/instrument-service.yaml` | `openapi/instrument-service.yaml` (attempt read and cancel from S05-T08; `POST /v1/instrument-tokens` and the `invalid_registration` and `unknown_provider` problem codes from S05-T09; S05-T10 and T11 add theirs) | 1.0.0 |
...
| Uncertain-outcome resolution and restart | `ResolverFakeCardIT`, `ResolverFakeBankIT`, restart test (S05-T12) | Not run | — | — |
| M8(b) scale | `UnknownOutcomeScaleIT` with recorded seed (S05-T12) | Not run | — | — |
| M8(c) no stuck attempts | `QuiesceNoStuckAttemptsIT` (S05-T12) | Not run | — | — |
| Alert-signal gauges | `AlertSignalGaugesIT` (S05-T12) | Not run | — | — |
