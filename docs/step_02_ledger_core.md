...
| SP1 study parameters | `services/ledger-service/src/test/resources/sp1/sp1.properties`, quoted in the report | D02-10 |
...
- [x] M6 (a): every changelog row returned by the API links to its money order and source idempotency key. *(`ChangelogApiIT.everyRowCarriesItsOrderAndSourceIdempotencyKey`: every row carries a UUID `order_id` and a non-blank source system and idempotency key, joined from `applied_orders` per §0.3 C17.)*
- [x] M6 (b): verify rebuilds balances from the changelog, matches stored balances and detects each tamper class. *(`VerifyApiIT` rebuilds balances from the changelog and compares them with stored accounts; a tampered delta, a tampered running balance and a removed row are each detected, and `HashChainTamperIT` adds insertion and reordering.)*
- [x] M6 (c): the ledger calls of the audit walk are documented and tested. M6 (c) is not a G1 condition; S04's end-to-end tests verify it (§0.3 E11 in [docs/zerosum_ledger_mvp_plan.md#decomposition-clarifications](zerosum_ledger_mvp_plan.md#decomposition-clarifications)). *(Not a G1 condition. The ledger calls are documented in the `openapi/ledger-service.yaml` operation descriptions and tested by `ChangelogApiIT`; S04 verifies the end-to-end walk per §0.3 E11.)*
...
- [x] I2–I4 queries run under the read-only verifier role (TB5), and I5 verification passes for every entity in the stress run ([docs/zerosum_ledger_mvp_plan.md#should-have](zerosum_ledger_mvp_plan.md#should-have) S1), or D02-6 records the minimum cut. *(The stress run reads I2–I4 through a `verifier`-role connection and verifies I5 for every entity — 208 entities, 6 405 and 6 411 rows, both modes. The standalone `InvariantQueriesVerifierRoleIT` for the D02-8 SQL units arrives with S02-T05.)*
...
- [x] The invariants endpoint reports the quarantine count and non-zero clearing balances only (§0.3 C13), and invariants and verify run under the D02-8 per-transaction timeout (§0.3 C19). *(`InvariantsApiIT`: unresolved-only count per D02-9, non-zero clearing balances classified by D01-6 via CR-S02-04, and both reads run under the D02-8 operational timeout in one snapshot.)*
- [x] `openapi/ledger-service.yaml` covers balances, changelog, invariants and verify, and `LedgerOpenApiContractIT` passes. *(All four operations are in the file and `LedgerOpenApiContractIT` validates real responses for each.)*
- [x] SP1 is recorded in `docs/results/sp1-lock-study.md` per [docs/zerosum_ledger_mvp_plan.md#spikes](zerosum_ledger_mvp_plan.md#spikes) and [docs/zerosum_ledger_mvp_plan.md#perf-tests](zerosum_ledger_mvp_plan.md#perf-tests), with unexecuted runs marked "Not run". *(All 10 D00-9 sections filled, MEASURED and ESTIMATE labelled separately, raw per-window data committed alongside.)*
- [x] D02-10 records the S3 must-have decision against the SP1 criterion. *(Ceiling 606.8 orders/s against the master criterion; S3 remains should-have and `default-batch-size: 1` stands.)*
- [x] ADR-0004 (ledger half) and ADR-0005 are committed, and ADR-0005 cites the SP1 report ([docs/zerosum_ledger_mvp_plan.md#decisions](zerosum_ledger_mvp_plan.md#decisions)). *(ADR-0005 consequences now carry the measured ceiling and link the report.)*
- [x] G1 has been evaluated against every clause in [docs/zerosum_ledger_mvp_plan.md#decision-gates](zerosum_ledger_mvp_plan.md#decision-gates) (the v1.2 proceed clause per §0.3 E11, narrow scope, change architecture) and the result recorded in H.6. *(Evaluated clause by clause in H.6: proceed. The narrow-scope hours clause is recorded as not evaluable, because hours were never tracked.)*
- [x] Section H is filled with actual paths and evidence, I.1 statuses are current, and I.2 was refreshed before handoff. *(H.1 has all twelve D02 decisions non-Pending; H.2–H.4 carry actual paths and evidence; I.1 is current; I.2 was refreshed immediately before handoff.)*
...
| D02-6 | **Canonical form version 1**, specified in `ChangelogHasher` and used by both apply and verification: SHA-256 over, in fixed order, the form version as one byte; a presence byte for the previous hash (0 for the first row of an entity, otherwise 1 followed by its 32 bytes); the entity ID as a 4-byte big-endian length then UTF-8 bytes; the sequence number as 8 bytes big-endian; the order ID as 16 bytes, most significant half first; the account code and the currency each length-prefixed the same way; the delta and the balance after the entry each as 8 bytes big-endian two's complement. Every field is length-prefixed or fixed-width, so no boundary is ambiguous and nothing is delimiter-joined. **`recorded_at` is excluded**: hashing it would force a read-back of the database-assigned value inside the apply transaction, and it fixes no audit fact the other fields do not. **Chain scope is the entity:** one chain across all of that entity's accounts and currencies, following the per-entity sequence. **Versioning:** `entity_changelog.hash_version` per row; a verifier meeting a version it does not implement reports `UNSUPPORTED_FORM_VERSION` instead of guessing. **Verifier:** `ChainVerifier.verify(rows in sequence order)` returns the first failing sequence number, a typed failure (`SEQUENCE_BREAK`, `UNSUPPORTED_FORM_VERSION`, `BROKEN_LINK`, `HASH_MISMATCH`) and the rows checked. Checks run in a fixed order within each row — sequence continuity, then form version, then the previous-hash link, then the recomputed hash — so the reported class is deterministic rather than incidental: reordering two rows surfaces as `BROKEN_LINK` at the first displaced sequence, because the displaced row's link is read before its hash is recomputed. I5 and the S02-T05 verify endpoint both call it; apply shares the same hasher, so writer and reader cannot drift. Per instruction 6 the number reported is the first sequence where a check fails, so a row modified *with its own hash recomputed* is reported at its successor, whose link is stale. **Vectors:** `services/ledger-service/src/test/resources/hash-chain/known-answer.json`, 7 rows covering a first row, a multi-currency entity and negative deltas and balances; the digests were produced by an independent reference implementation written from this specification, so `HashChainKnownAnswerTest` is a cross-implementation check rather than the hasher confirming itself | Length-prefixed, fixed-order bytes make the form reproducible in another language, which is what lets S06 and S09 check themselves against the vectors; excluding `recorded_at` keeps apply to one round trip; one shared implementation means the chain cannot drift between writer and reader | Delimiter-joined text (ambiguous as soon as a field can contain the delimiter); hashing the row as JSON (key order and number formatting are not stable across libraries); including `recorded_at` (a read-back inside the transaction for no audit gain); a separate verifier implementation for reading (would drift from the writer) | Accepted | 2026-09-15 |
| D02-7 | **Contract:** `openapi/ledger-service.yaml` (OpenAPI 3.1, so its schemas are JSON Schema 2020-12 and the D00-1 validator checks them directly). It carries the two read operations only; paths added later by other steps, for example the freshness endpoint (D04-5), are owned by the decision that introduces them, and the invariants and verify operations are added by S02-T05. Errors are RFC 9457 `application/problem+json` with a stable `code` that callers branch on instead of the title: `invalid_entity_id`, `invalid_cursor`, `invalid_limit`, `entity_not_found`, `unauthorized`, `forbidden`, `database_unavailable`. 401 and 403 are declared but not enforced; S03-T03 wires `libs/auth` (§0.3 C9) and until then the port stays local-only. **Balances:** one statement joining the entity to its accounts, so `as_of_seq` and the balances come from the same snapshot without holding a transaction open. Each account returns `normal_side`, `presented_minor` on that side (ADR-0003, D01-6) **and** `signed_minor` as stored, so a caller can reconcile against the raw ledger without re-implementing the chart. An entity with no accounts yet returns an empty list with `as_of_seq` 0. **Changelog:** keyset paging over the `(entity_id, seq)` primary key with `after_seq` and `limit`; `ledger.api.default-page-size` 100 and `ledger.api.max-page-size` 500 live in configuration with a D02-7 trace comment, so the master's ceiling is never duplicated in code. Each row carries the sequence, order ID, account, currency, delta, running balance, recorded time and the source system and idempotency key joined from `applied_orders` (§0.3 C17) — that join is the M6 (a) link. **Hash fields are not exposed:** tamper evidence is what verify reports (D02-6), and the S09 Explorer shows verify results rather than raw hashes. A request past the last sequence returns an empty page with no cursor, never 404. **Unknown entity is 404 on both operations**, so they answer alike. **Serialization:** `spring.jackson.property-naming-strategy: SNAKE_CASE` once, rather than annotating every response record; every schema sets `additionalProperties: false`, so `LedgerOpenApiContractIT` fails on any naming slip. The page query lives in one shared constant used by both the store and the execution-plan test, so the recorded plan evidence cannot drift from the statement the service runs. **Audit walk (M6 (c))** is documented in the operation descriptions: balances give `as_of_seq`, the changelog pages the rows behind it, and each row's source idempotency key is the handle for the order in order-service; the end-to-end walk is verified in S04 (§0.3 E11) | One snapshot statement is cheaper than a held repeatable-read transaction for a single-entity read; returning both the presented and the signed value keeps the chart out of callers; configuration-held page bounds keep the master's numbers out of the code; withholding hash fields stops clients re-implementing verification that D02-6 owns | A repeatable-read transaction for balances (a second round trip and a held snapshot for one row); returning only the presented value (callers could not reconcile against stored balances); 200 with an empty body for an unknown entity (would hide a typo in an entity ID); exposing `prev_hash` and `row_hash` (invites clients to re-implement chain verification); offset paging (drifts as rows are appended); annotating every DTO for snake_case (repetitive, and one setting is already covered by the contract test) | Accepted. S02-T05 added the invariants and verify operations to the same file, completing D02-7 | 2026-09-15 |
| D02-8 | **Units:** `InvariantQueries` holds I2 (global per-currency sum), I3 (balance equals the sum of deltas **and** every stored running balance is correct) and I4 (gapless sequence per entity) as standalone SELECTs with no web or apply dependency, so S06-T04 reuses them unchanged; `InvariantQueriesVerifierRoleIT` proves they run under the read-only `verifier` role (TB5). I3 is deliberately two checks, because stored balances can agree with the deltas while one row's `balance_after_minor` is wrong. **Snapshot:** `InvariantsService` runs every query for one response inside a single read-only REPEATABLE READ transaction, so all figures describe the same state while applies commit. **Operational timeout (§0.3 C19):** `ledger.api.operational-statement-timeout` is **15 s**, set as `SET LOCAL` inside the read transaction only; the global 5 s apply limit is unchanged. Chosen against the master §6.2 P4 targets — invariants over 1M changelog rows ≤ 10 s and verify over 10 000 rows ≤ 2 s — with headroom on a busy database, and deliberately not so large that a stuck read holds a snapshot indefinitely. **Quarantine count:** unresolved rows only (`resolved_at IS NULL`, D02-9); a resolved row leaves the count. **Clearing report:** non-zero clearing balances only, classified by `ChartOfAccounts.isClearing` / `clearingAccounts()` (**CR-S02-04 to D01-6**, raised because D01-6 had no such classification and ledger-service must not carry account codes). No age or settlement-cycle filter: the S06 verifier judges age and break matching for I9. **I5 across all entities (instruction 6):** *not* evaluated on the invariants endpoint. Verifying every chain rehashes the whole changelog and cannot meet the P4 budget on a large ledger, so the response carries an explicit `i5: {evaluated: false, reason}` rather than an absent field that a reader could mistake for a pass; per-entity verify and the S06 verifier own I5, and the OpenAPI description says so. **Verify:** one read-only REPEATABLE READ transaction streams the entity's changelog in sequence order at `ledger.api.verify-fetch-size` 500 rows, so a very large entity is never materialized; it recomputes each account's running balance, compares every row and the rebuilt totals against stored balances, delegates the chain to the D02-6 `ChainVerifier`, and reports the **lowest** failing sequence across all checks. A read that exceeds the timeout returns a problem with code `operational_read_timeout`, never a `consistent` result | Standalone units keep the invariants reusable by the verifier and runnable under a read-only role; one snapshot stops applies producing false inconsistencies; naming I5 as not evaluated is honest where silence would read as a pass; streaming keeps verify bounded on the hot entities that matter | Evaluating I5 for every entity in the endpoint (cannot meet P4 on a large ledger); omitting the I5 field entirely (a reader would infer it passed); reusing the global 5 s statement timeout (too short for the P4 invariants target, §0.3 C19); raising the global timeout instead (would loosen the apply path as well); materializing the changelog in verify (unbounded memory on a hot entity); hard-coding clearing account codes in ledger-service (rejected in favour of CR-S02-04 to the owner) | Accepted | 2026-09-15 |
...
| D02-10 | **SP1 result (MEASURED).** Study: 12 windows of 180 s with a 120 s warm-up each, at 1, 4, 12 and 32 writers x 3 repetitions, per-order mode, every order touching `platform:main`, on the reference machine against a database configured as D00-3 describes **with durability intact** (ordinary ledger tests run `fsync=off`, which would have made the number a smoke check). Medians of three repetitions: 1 writer **545.8**, 4 writers **384.8**, 12 writers **451.3**, 32 writers **606.8** orders/s. **Ceiling = 606.8 orders/s** (highest median across writer counts; that count's repetitions ranged 577.5–665.8, entirely above the criterion, so the ranges do not straddle it). Going from 1 to 32 writers bought 1.11x throughput for 8.8x apply p50, with lock-wait p95 rising from ~0.1 ms to 59–75 ms. Every window was a valid data point: zero deadlock, lock-timeout, connection and exhausted-retry events, and zero I2–I5 violations. **S3 decision:** the criterion at [docs/zerosum_ledger_mvp_plan.md#spikes](zerosum_ledger_mvp_plan.md#spikes) promotes batched apply to must-have only if the ceiling falls below it; the measured ceiling is above it, so **S3 remains a should-have** and D02-3's `default-batch-size: 1` stands unchanged for S04. The S07-T06 starting point is likewise unchanged. **Not concluded here:** this is apply-only throughput and says nothing on its own about T1 end-to-end, which S07 measures | A measured ceiling beats an estimate, and the decision rule is the master's, applied to the median-of-repetitions definition stated in the report | Promoting S3 anyway (spends S04 effort the measurement does not justify); reporting the best window instead of the median (flatters the result and ignores the master's variance rule); measuring against `fsync=off` (faster, and wrong) | Accepted | 2026-09-15 |
...
|---|---|---|---|
| Ledger migrations (schema, append-only triggers, grants) | `services/ledger-service/src/main/resources/db/migration/` | `services/ledger-service/src/main/resources/db/migration/V2__ledger_schema.sql` (`b8d6124`) | D02-1, D02-2, D02-9 |
| Append-only assertion test helper | ledger-service test-fixtures source set (per D00-10) | `services/ledger-service/src/test/java/dev/zerosum/ledger/support/AppendOnlyAssertions.java` — the test source set, since D00-10 defines no test-fixtures set for services (`b8d6124`) | D02-2 |
| Apply engine, decoding, retry classifier | `services/ledger-service/src/main/java/<package-root>/apply/` | `services/ledger-service/src/main/java/dev/zerosum/ledger/apply/` (`482c221`) | D02-3, D02-4, D02-5, D02-12 |
| SQL access layer | `services/ledger-service/src/main/java/<package-root>/store/` | `services/ledger-service/src/main/java/dev/zerosum/ledger/store/` (`482c221`) | D02-1, D02-3 |
| Changelog hasher and chain verifier | `services/ledger-service/src/main/java/<package-root>/changelog/` | `services/ledger-service/src/main/java/dev/zerosum/ledger/changelog/` (`482c221`; `ChainVerifier` added by S02-T06) | D02-6 |
| Invariant SQL units | `services/ledger-service/src/main/java/<package-root>/invariants/` | `services/ledger-service/src/main/java/dev/zerosum/ledger/invariants/` (`InvariantQueries`, `InvariantsService`, `VerifyService`) | D02-8 |
| Read API controllers | `services/ledger-service/src/main/java/<package-root>/api/` | `services/ledger-service/src/main/java/dev/zerosum/ledger/api/` (`LedgerReadController`, `LedgerReadResponses`, `LedgerApiException`, `LedgerApiExceptionHandler`, `LedgerApiProperties`) | D02-7 |
| Apply timeouts, retry bounds, default batch mode, pool size | `services/ledger-service/src/main/resources/application.yml` | same (`0130cde`, `6cb70b8`, `2d762a3`, `482c221`) | D02-3, D02-4 |
| Changelog page size, invariants and verify timeout override | `services/ledger-service/src/main/resources/application.yml` | `ledger.api.default-page-size` and `ledger.api.max-page-size` set by S02-T04; the invariants and verify timeout override is added by S02-T05 (D02-8) | D02-7, D02-8 |
| Test driver | ledger-service test-fixtures or test source set (per D00-10) | `services/ledger-service/src/test/java/dev/zerosum/ledger/support/ApplyTestDriver.java` (test source set) (`482c221`) | D02-3 |
...
| SP1 study configuration | `services/ledger-service/src/test/resources/sp1/` | `services/ledger-service/src/test/resources/sp1/sp1.properties` | D02-10 |

### H.3 Produced artifacts

| Artifact | Planned path | Actual path | Revision/hash |
|---|---|---|---|
| Ledger read API specification | `openapi/ledger-service.yaml` | same — all four operations: balances, changelog, invariants and verify | balances and changelog committed with S02-T04; invariants and verify with S02-T05 |
| ADR-0004 balance policy outside the ledger | `docs/adr/0004-balance-policy-outside-ledger.md` | same (Accepted) | `482c221` |
| ADR-0005 ledger entity locks and retries | `docs/adr/0005-ledger-entity-locks.md` | same (Accepted) | `482c221`; revised by S02-T03 (decision points 3 and 4) |
| SP1 lock study report | `docs/results/sp1-lock-study.md` | same (all 10 D00-9 template sections) | committed with S02-T07 |
| SP1 raw run data | `docs/results/sp1/` | `docs/results/sp1/sp1-runs.json` (one record per window, written as each window completed) | committed with S02-T07 |
| Hash-chain known-answer vectors | `services/ledger-service/src/test/resources/hash-chain/known-answer.json` | same | committed with S02-T06 |
| Ledger integration, stress and API test suites | `services/ledger-service/src/test/` (and integration source set per D00-10) | `services/ledger-service/src/test/java/dev/zerosum/ledger/{schema,apply,changelog,stress,support}/` — the test source set, tagged per D00-10; API suites arrive with S02-T04 and S02-T05 | `b8d6124`, `482c221`; stress committed with S02-T03 |
...
| Balances and changelog APIs | `BalancesApiIT`, `ChangelogApiIT` | **Passed** (5 + 4 tests), the first Spring-context tests in the repository. Balances: after golden O1–O7 every account matches the D01-9 expected values on **both** `presented_minor` (the account's normal side) and `signed_minor`; `as_of_seq` equals the entity's changelog row count; `driver:D1` `payable` reports `CREDIT`; an unknown entity is 404 `entity_not_found` and a malformed one 400 `invalid_entity_id`; a literal and a percent-encoded colon address the same entity. Changelog: paging with `limit=7` over 25 orders returns every row exactly once in sequence order with no repeats; every row carries a UUID `order_id` and a non-blank source system and idempotency key (the M6 (a) link); a request past the last sequence is an empty page with no cursor, not a 404; `limit` 0, 501 and `abc` and `after_seq` −1 and `abc` are each 400 with the stable code. **Two failures along the way were both test bugs, not service bugs:** Boot 4.1 has no `RestClient.Builder` bean without its client module, and `RestClient.uri(String)` re-encoded the `%` so the encoded-colon case had never actually been sent | [docs/results/s02/s02-t04-read-apis.txt](results/s02/s02-t04-read-apis.txt) | 2026-09-15 |
| Changelog page query uses the primary-key range | `ChangelogPagePlanIT` (EXPLAIN on the real statement) | **Passed**: with 500 orders applied to one entity and both tables analysed, the plan is `Index Scan using entity_changelog_pkey on entity_changelog c` with `Index Cond: ((entity_id = 'rider:R_PLAN'::text) AND (seq > 250))`, joined to `applied_orders` by `Index Scan using applied_orders_pkey` under a nested loop. The test asserts the primary-key index is used and that `entity_changelog` is never sequentially scanned. The statement is a constant shared with `LedgerStore`, so this evidence cannot drift from the query the service runs | [docs/results/s02/s02-t04-read-apis.txt](results/s02/s02-t04-read-apis.txt) | 2026-09-15 |
| OpenAPI contract | `LedgerOpenApiContractIT` | **Passed** (3 tests): real balances, changelog and problem responses from the running service validate against `openapi/ledger-service.yaml`, using the same pinned JSON Schema validator as D01-8 rather than a second one. The spec is OpenAPI 3.1, so its schemas are JSON Schema 2020-12 and are checked directly; each response is validated against a document whose root is the operation's own `$ref` with the whole `components` block attached, so internal references resolve offline. Every schema sets `additionalProperties: false`, which is what checks the snake_case serialization field by field | [docs/results/s02/s02-t04-read-apis.txt](results/s02/s02-t04-read-apis.txt) | 2026-09-15 |
| Invariants endpoint and verify | `InvariantsApiIT`, `VerifyApiIT` | **Passed** (4 + 6 tests). Invariants, driven through one database from clean to corrupted: after O1–O2 the card charge shows as a non-zero `provider:fakecard` clearing balance; after the full O1–O7 the report is consistent with I2, I3 and I4 empty, quarantine 0 and **every clearing account back to zero**; an undecodable record makes the unresolved count 1, and setting `resolved_at` returns it to 0 (D02-9); with the append-only triggers bypassed by a superuser to corrupt a delta and delete a row, the report is **not** consistent and names both I3 and I4 violations. `i5.evaluated` is false with a reason, so the endpoint never implies I5 passed. Verify, one entity per tamper case: a clean entity is consistent with `rows_checked` equal to its changelog length; a tampered delta, a tampered running balance and a removed row are each detected at the expected sequence; verify repeated during concurrent applies never reports a half-applied state; an unknown entity is 404 | [docs/results/s02/s02-t05-invariants-verify.txt](results/s02/s02-t05-invariants-verify.txt) | 2026-09-15 |
| I2–I4 under the verifier role | `InvariantQueriesVerifierRoleIT` | **Passed** (2 tests): I2, I3, I4, the unresolved quarantine count and the clearing report all run under the read-only `verifier` role (TB5, D00-4), which is what lets the S06 verifier reuse the units with no write access. The clearing report is classified by `ChartOfAccounts.isClearing` (CR-S02-04), and the test asserts only clearing accounts with non-zero balances appear | [docs/results/s02/s02-t05-invariants-verify.txt](results/s02/s02-t05-invariants-verify.txt) | 2026-09-15 |
| Hash chain known answers and tamper detection | `HashChainKnownAnswerTest`, `HashChainTamperIT` | **Passed** (3 + 7 tests). The 7 known-answer vectors were computed by an **independent reference implementation** written from the D02-6 specification, so the test is a cross-implementation check rather than the hasher confirming itself; a third test asserts the vectors keep their required coverage (an entity's first row, a multi-currency entity, a negative delta and a negative balance). **Negative control:** flipping one hex digit of a stored vector hash fails two of the three tests with the expected message; the file restored byte-identical and the test is green again. Tamper detection, each via a superuser session with the D02-2 triggers disabled: modified field `HASH_MISMATCH` at seq 2; modified row with its hash recomputed `BROKEN_LINK` at seq 3; removed row `SEQUENCE_BREAK` at seq 3; removed and renumbered `BROKEN_LINK` at seq 3; inserted row `BROKEN_LINK` at seq 5; reordered rows `BROKEN_LINK` at seq 2; and a clean chain verifies. The reorder case corrected a wrong test expectation, not the verifier | [docs/results/s02/s02-t06-hash-chain.txt](results/s02/s02-t06-hash-chain.txt) | 2026-09-15 |
| Informal verify and invariants timing (context only) | `OperationalReadTimingIT` on a generated entity with 10 000 changelog rows | **Measured, context only:** verify streamed all 10 000 rows in **36 ms** and the invariants report took **46 ms** (seeding took 2 314 ms). The container runs with `fsync=off` and nothing else was running, so these are not P4 measurements — P4 is measured in S07 on the reference environment and is not gated here. They say only that neither operational read is pathological at the P4 reference size | [docs/results/s02/s02-t05-invariants-verify.txt](results/s02/s02-t05-invariants-verify.txt) | 2026-09-15 |
| SP1 lock study | `Sp1LockStudy` (tag `study`, no CI job runs it) and the D00-9 report | **Measured**: 12 windows of 180 s plus a 120 s warm-up each, at 1/4/12/32 writers x 3 repetitions, per-order, all orders on `platform:main`, durability intact on the reference machine. Medians 545.8 / 384.8 / 451.3 / 606.8 orders/s; **ceiling 606.8** with its repetition range 577.5–665.8 wholly above the criterion. Throughput x1.11 from 1 to 32 writers against apply p50 x8.8 and lock-wait p95 ~0.1 ms → 59–75 ms. All 12 windows valid: zero retries of every class, zero I2–I5 violations. MEASURED is 1.8–3x the master's ESTIMATE of 200–333 orders/s, because that estimate assumed a 3–5 ms lock hold where measured apply p50 at one writer is 0.51 ms. A first attempt was **discarded, not reported** (mis-forwarded overrides, then an OOM in the I5 sweep after two windows); the harness faults it exposed were fixed and the study rerun from scratch | [docs/results/sp1-lock-study.md](results/sp1-lock-study.md), raw data [docs/results/sp1/sp1-runs.json](results/sp1/sp1-runs.json) | 2026-09-15 |
...
| A chain rewritten together with the entity's stored head still verifies | limitation | The hash chain is tamper-evident only relative to an independently held head. Someone with owner or superuser rights who rewrites every `entity_changelog` row **and** `entities.last_hash` produces a chain that verifies. Row-local modification, removal, insertion and reordering are each detected (`HashChainTamperIT`) | Out of the MVP threat model, which covers application and operator mistakes. The S06 verifier's cross-store checks (I6, I6b) compare the ledger against the order store, which a ledger-only rewrite cannot satisfy; list in the S09 release limitations |
...
| Field | Value |
|---|---|
| Step status | **Done.** S02-T01 to S02-T07 are Done; S02-C01 stays Planned with its trigger not met. D02-1 to D02-12 are all recorded, and four change requests were raised and accepted (CR-S02-01 to D00-4, CR-S02-02 to D01-5, CR-S02-04 to D01-6, CR-S02-05 to D00-10) |
| Gate result | **G1: proceed.** Evaluated clause by clause below |
| Completed on | 2026-09-16 |
| Completed by | S02 implementation |
| Handoff accepted by next step | — (S03 records acceptance here) |

**G1 evaluation** against every clause of [docs/zerosum_ledger_mvp_plan.md#decision-gates](zerosum_ledger_mvp_plan.md#decision-gates).

| Clause | Result | Evidence |
|---|---|---|
| **Proceed —** M5 (b) gapless sequences | **Pass** | I4 clean after every stress window at the master's full size, both modes, and after each SP1 window ([s02-t03-stress.txt](results/s02/s02-t03-stress.txt), [sp1-lock-study.md](results/sp1-lock-study.md)) |
| **Proceed —** M5 (c) global per-currency sum is zero | **Pass** | The sampler read the sum throughout each stress run and every sample was zero, with I2 clean afterwards; the test asserts it sampled at least once, so a silent no-op sampler cannot pass |
| **Proceed —** M5 (d) balances equal changelog deltas | **Pass** | I3 clean, covering both the delta sums and every stored running balance |
| **Proceed —** M6 (a) changelog rows link to order and source key | **Pass** | `ChangelogApiIT` asserts a UUID order ID and a non-blank source system and idempotency key on every row ([s02-t04-read-apis.txt](results/s02/s02-t04-read-apis.txt)) |
| **Proceed —** M6 (b) verify rebuilds balances and detects tampering | **Pass** | `VerifyApiIT` detects a tampered delta, a tampered running balance and a removed row at the expected sequence; `HashChainTamperIT` adds insertion and reordering ([s02-t05-invariants-verify.txt](results/s02/s02-t05-invariants-verify.txt), [s02-t06-hash-chain.txt](results/s02/s02-t06-hash-chain.txt)) |
| **Proceed —** immutability enforced | **Pass** | 2 append-only tables x {UPDATE, DELETE, TRUNCATE} x {application, owner} all rejected with 42501, with a negative control that fails exactly the two owner cells ([s02-t01-schema.txt](results/s02/s02-t01-schema.txt)) |
| **Proceed —** generative and concurrency stress tests pass | **Pass** | `SeededApplyPermutationIT` green; `LedgerConcurrencyStressIT` green at 32 threads x 20 000 orders x 30% duplicates in both modes, zero retries of every class |
| **Proceed —** SP1 recorded | **Pass** | [docs/results/sp1-lock-study.md](results/sp1-lock-study.md), all 10 D00-9 sections, ceiling 606.8 orders/s, raw per-window data committed |
| **Proceed —** M6 (c) audit walk | **Deferred by the master (v1.2)** | Not a G1 condition. The ledger calls are documented in the OpenAPI descriptions and tested; S04's end-to-end tests verify the walk (§0.3 E11) |
| **Narrow scope if** cumulative hours > 75 (planned 60) | **Not evaluable from this step's records** | Wall-clock hours were not tracked: I.1 has no hours column and no actuals were recorded per task. The planned figure for S02 is 24 h (master §step-02). This is recorded as a gap rather than estimated, since an invented number could wrongly trigger or suppress the minimum cut |
| **Change architecture if** unresolvable deadlocks or sequence gaps | **Did not fire** | Two lock defects were found by the stress test's zero-deadlock assertion and fixed in place (ADR-0005 points 3 and 4); the final state has zero deadlock retries and zero I4 violations at full size, so S02-C01 was not triggered |
| **Stop or re-plan if** | **Not applicable** | The gate defines no stop condition for G1 |
...
| S02-T04 | Done | `openapi/ledger-service.yaml`, `services/ledger-service/src/main/java/dev/zerosum/ledger/api/{LedgerReadController,LedgerReadResponses,LedgerApiException,LedgerApiExceptionHandler,LedgerApiProperties}.java`, `…/store/LedgerStore.java` (`readBalancesSnapshot`, `readChangelogPage`, shared `CHANGELOG_PAGE_SQL`), `application.yml` (`ledger.api.*`, `spring.jackson.property-naming-strategy`), `…/src/test/java/dev/zerosum/ledger/api/{LedgerApiTestBase,BalancesApiIT,ChangelogApiIT,LedgerOpenApiContractIT}.java`, `…/store/ChangelogPagePlanIT.java`, `build.gradle.kts` (validator on the test classpath) | [docs/results/s02/s02-t04-read-apis.txt](results/s02/s02-t04-read-apis.txt): 13 API and plan tests green in a 79-test suite, responses validated against the specification, and the keyset plan captured from the shared SQL constant | None. The section G OpenAPI item stays unticked because it also requires the invariants and verify operations, which S02-T05 adds to the same file |
| S02-T05 | Done | `services/ledger-service/src/main/java/dev/zerosum/ledger/invariants/{InvariantQueries,InvariantsService,VerifyService}.java`, `…/api/{LedgerVerificationController,EntityIds}.java` and additions to `LedgerReadResponses` and `LedgerApiExceptionHandler`, `openapi/ledger-service.yaml` (invariants and verify), `application.yml` (`operational-statement-timeout`, `verify-fetch-size`), `libs/money` `ChartOfAccounts` clearing classification (CR-S02-04); tests `InvariantsApiIT`, `VerifyApiIT`, `InvariantQueriesVerifierRoleIT`, `OperationalReadTimingIT` and two more contract cases | [docs/results/s02/s02-t05-invariants-verify.txt](results/s02/s02-t05-invariants-verify.txt): 14 new tests (4 + 6 + 2 plus two contract cases) green on their first run in a 93-test suite; `OperationalReadTimingIT` was added afterwards and takes the suite to 94 | None. CR-S02-04 was raised to D01-6 and accepted before ledger-service used the classification |
| S02-T06 | Done | `services/ledger-service/src/main/java/dev/zerosum/ledger/changelog/{ChangelogHasher,ChainVerifier}.java`, `…/store/LedgerStore.java` (`readChangelog`, `allEntityIds`), `…/src/test/resources/hash-chain/known-answer.json`, `…/src/test/java/dev/zerosum/ledger/changelog/{HashChainKnownAnswerTest,HashChainTamperIT}.java`, full chain verification in `LedgerConcurrencyStressIT` | [docs/results/s02/s02-t06-hash-chain.txt](results/s02/s02-t06-hash-chain.txt): vectors cross-checked against an independent implementation with a negative control, six tamper classes each detected at the expected sequence, and I5 verified for all 208 entities (6 405 and 6 411 rows) in both stress modes | None. The chain is tamper-evident only against an independently held head (H.5) |
| S02-T07 | Done | `services/ledger-service/src/test/java/dev/zerosum/ledger/sp1/{Sp1LockStudy,Sp1Parameters,Sp1Run,Window}.java`, `…/src/test/resources/sp1/sp1.properties`, `…/support/LedgerTestDatabase.java` (`startDurable`), `…/store/LedgerStore.java` (`streamChangelog`), the `build-logic` `study` layer (CR-S02-05), `docs/results/sp1-lock-study.md`, `docs/results/sp1/sp1-runs.json`, ADR-0005 consequences | [docs/results/sp1-lock-study.md](results/sp1-lock-study.md): ceiling 606.8 orders/s, all 12 windows valid, D02-10 decided against the master's criterion | None. CR-S02-05 to D00-10 was raised and accepted before the runner used the `study` tag |
| S02-C01 | Planned — trigger not met | — | The contingency switch to account-level optimistic locking is triggered by unresolvable deadlocks or sequence gaps. Neither occurred: `LedgerConcurrencyStressIT` reports zero deadlock retries and zero I4 violations at the master's full size in both modes ([docs/results/s02/s02-t03-stress.txt](results/s02/s02-t03-stress.txt)), and all 12 SP1 windows were clean ([docs/results/sp1-lock-study.md](results/sp1-lock-study.md)). Two lock defects were found and **fixed** rather than routed to this contingency (ADR-0005 decision points 3 and 4) | — |
...
|---|---|---|---|---|
| `docs/zerosum_ledger_mvp_plan.md` | `1df78b3 2026-09-15` | 2026-09-15 (S02 start at `2a5233a`; rechecked at the Phase 1 and Phase 2 boundaries and again before handoff) | All | No impact: unchanged at all three rechecks |
...
| Consumed artifacts, digest check before handoff | `libs/money` `sha256 d50a89254b266541`, `libs/contracts` `sha256 6f80eabfd4959492`, `docker-compose.yml` `sha256 55d1cc64293788ae`, `gradle/libs.versions.toml` `sha256 6c95b50f2dd4996a` (directory digests per the change-detection procedure) | 2026-09-16 (before handoff) | All | `libs/money` changed only through CR-S02-04, which this step raised and revalidated. `libs/contracts`, the Compose file and the version catalog are unchanged since S02 started, so no pinned version or contract moved under this step |
| **CR-S02-05 → D00-10** (change request) | Need: S02-T07's outputs require the SP1 runner to be tagged per D00-10 and **excluded from CI**, but D00-10 defines only `integration` and `e2e`, and the untagged `test` layer excludes just those two. A `@Tag("study")` class would therefore have run inside `./gradlew build` on every push, adding roughly an hour of measurement windows to CI. **Resolution:** `zs.java-conventions` adds a third layer — `study` is excluded from the untagged `test` task and gets its own `studyTest` task, registered by the same mapping that already creates `integrationTest` and `e2eTest`. No CI job invokes `studyTest`, so the study runs only when started deliberately and its output is committed as evidence. **Impact:** one convention plugin file; no existing test changes layer, and `build`, `integrationTest` and `e2eTest` behave exactly as before. No acceptance gate changes | 2026-09-15 (S02-T07 start) | S02-T07; S00-T02 (conventions) revalidated by the full build | Change request raised and accepted; the owning register is updated in S00 before the runner uses the tag |
| **CR-S02-04 → D01-6** (change request) | Need: S02-T05 instruction 5 requires the non-zero clearing report to use the D01-6 clearing classification and forbids hard-coding account codes in ledger-service, but `ChartOfAccounts` exposed only `kindOf`, `isAllowed`, `allowedAccounts`, `normalSide` and `presentOnNormalSide` — `clearing` and `payout_clearing` existed only inside private maps. ADR-0003 already treats a non-zero clearing balance after quiesce as the I9 signal, so the classification was implied by D01-6 but unreachable. **Resolution:** the owner exposes `ChartOfAccounts.isClearing(account)` and `clearingAccounts()`, covered by a new `ChartOfAccountsTest` case that also asserts every clearing code is allowed for the provider kind. **Impact:** `libs/money` (one class, one test); S01 D01-6 register row updated and S01-T03 revalidated; ledger-service consumes the API in S02-T05 and carries no account codes of its own. No other step is affected, and no acceptance gate changes | 2026-09-15 (S02-T05 start) | S02-T05; S01-T03 revalidated | Change request raised and accepted; the owning register was updated before ledger-service used it |
...
| `docs/README.md` | — | — | All | — |
| `docs/step_00_foundations.md` (register H) | `1951e7b 2026-09-15`; working tree `sha256 e2054a597adf69bd…` at the Phase 2 boundary | 2026-09-15 (S02 start; rechecked at the Phase 2 boundary) | All | Revalidated twice. At the Phase 2 boundary the file had changed only in its S00-T05 CI evidence and status rows once the public repository was authorized. Before handoff it is at `b4de2cb`, changed again by **CR-S02-05**, which this step raised: D00-10 gained the `study` test layer and S00-T02 was revalidated by a full build. Both changes are this step's own accepted change requests, so no S02 task needs review |
| `docs/step_01_domain_contracts.md` (register H) | `de3db72 2026-09-15`; working tree `sha256 2eca89fc69ab176c…` at the Phase 2 boundary | 2026-09-15 (S02 start; rechecked at the Phase 2 boundary) | S02-T02 to S02-T07 | Revalidated twice. At the Phase 2 boundary only the CI placement evidence row had changed. Before handoff the file is at `f0c9240`, changed by **CR-S02-04**, which this step raised: D01-6 gained the clearing classification and S01-T03 was revalidated at 83 `libs/money` tests green. `libs/contracts` is unchanged. Both changes are this step's own accepted change requests, so no S02 task needs review |
