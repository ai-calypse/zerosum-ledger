...
| Stress harness parameters | `services/ledger-service/src/test/resources/stress/` (planned) | D02-11 |
...
- [ ] M5 (b): changelog sequence numbers are gapless per entity in `LedgerConcurrencyStressIT` and in the I4 query ([docs/zerosum_ledger_mvp_plan.md#must-have](zerosum_ledger_mvp_plan.md#must-have)).
- [ ] M5 (c): every sampled global per-currency sum during the stress run, and after it, is zero (I2 in [docs/zerosum_ledger_mvp_plan.md#invariants](zerosum_ledger_mvp_plan.md#invariants)).
- [ ] M5 (d): every account balance equals the sum of its changelog deltas, with correct running balances (I3).
- [ ] Ledger-level duplicate application (sequential, concurrent, within a batch) yields identical balances, supporting the M5 (a) criterion that S04 proves end-to-end.
...
- [ ] Immutability enforced: every append-only table rejects update, delete and truncate for the application and owner roles (§0.3 O3; G1 proceed condition in [docs/zerosum_ledger_mvp_plan.md#decision-gates](zerosum_ledger_mvp_plan.md#decision-gates)).
- [ ] Worked example O1–O7 and the O8 variant applied through the engine match the D01-9 expected balances ([docs/zerosum_ledger_mvp_plan.md#worked-example](zerosum_ledger_mvp_plan.md#worked-example)).
- [ ] Seeded generative apply with duplicates and reordering passes, with seeds reported per ADR-0009 ([docs/zerosum_ledger_mvp_plan.md#test-layers](zerosum_ledger_mvp_plan.md#test-layers)).
- [ ] The concurrency stress test runs at the parameters in [docs/zerosum_ledger_mvp_plan.md#test-layers](zerosum_ledger_mvp_plan.md#test-layers) (or the recorded D02-11 PR/nightly split), with zero exhausted retries and every deadlock retry explained.
...
- [ ] The apply engine writes quarantine rows under the surrogate key, including undecodable records, a duplicate quarantine insert for the same Kafka position is a no-op, and the quarantine table accepts resolution updates (§0.3 C8, C18, C25).
...
|---|---|---|---|
| Ledger migrations (schema, append-only triggers, grants) | `services/ledger-service/src/main/resources/db/migration/` | — | D02-1, D02-2, D02-9 |
| Append-only assertion test helper | ledger-service test-fixtures source set (per D00-10) | — | D02-2 |
| Apply engine, decoding, retry classifier | `services/ledger-service/src/main/java/<package-root>/apply/` | — | D02-3, D02-4, D02-5, D02-12 |
| SQL access layer | `services/ledger-service/src/main/java/<package-root>/store/` | — | D02-1, D02-3 |
...
| Apply timeouts, retry bounds, default batch mode, pool size | `services/ledger-service/src/main/resources/application.yml` | — | D02-3, D02-4 |
...
| Test driver | ledger-service test-fixtures or test source set (per D00-10) | — | D02-3 |
| Stress harness configuration | `services/ledger-service/src/test/resources/stress/` | — | D02-11 |
...
| ADR-0004 balance policy outside the ledger | `docs/adr/0004-balance-policy-outside-ledger.md` | — | — |
| ADR-0005 ledger entity locks and retries | `docs/adr/0005-ledger-entity-locks.md` | — | — |
...
| Ledger integration, stress and API test suites | `services/ledger-service/src/test/` (and integration source set per D00-10) | — | — |
...
| Worked example O1–O7 and O8 balances | `WorkedExampleApplyIT` | Not run | — | — |
| Duplicate application is a no-op | `ApplyDuplicateIT` | Not run | — | — |
| Seeded permutations with duplicates | `SeededApplyPermutationIT` | Not run | — | — |
| Retry classification, lock-timeout retry, exhausted retries | `ApplyRetryClassifierTest`, `ApplyLockTimeoutRetryIT`, `ApplyExhaustedRetriesIT` | Not run | — | — |
| Connection loss during the transaction | `ApplyConnectionLossIT` | Not run | — | — |
| Quarantine and overflow isolation | `ApplyQuarantineIT`, `ApplyOverflowIT` | Not run | — | — |
| ADR-0004 negative balances allowed | `NegativeBalanceIT` | Not run | — | — |
| Concurrency stress, both modes, retry counts by class | `LedgerConcurrencyStressIT` | Not run | — | — |
...
|---|---|---|---|---|
| `docs/zerosum_ledger_mvp_plan.md` | `d65384c 2026-09-15` | 2026-09-15 (S02 start at `2f5dbcf`, Phase 1 boundary) | All | No impact: unchanged |
...
| `docs/README.md` | — | — | All | — |
| `docs/step_00_foundations.md` (register H) | — | — | All | — |
| `docs/step_01_domain_contracts.md` (register H) | — | — | S02-T02 to S02-T07 | — |
