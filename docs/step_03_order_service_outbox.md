...
- [x] M2(a): `UPDATE`, `DELETE` and `TRUNCATE` on order tables fail for the app and owner roles ([docs/zerosum_ledger_mvp_plan.md#must-have](zerosum_ledger_mvp_plan.md#must-have)); evidence recorded in H.4.
- [x] M2(b): an unbalanced insert that bypasses the application fails at `COMMIT`. Header-only and per-currency imbalance cases also fail.
- [x] M2(c): an adjustment must reference an existing order, and the same-group check (§0.3 C2) is tested.
- [x] M3(a)–(e) pass through the HTTP API exactly as worded in [docs/zerosum_ledger_mvp_plan.md#must-have](zerosum_ledger_mvp_plan.md#must-have), including the concurrent same-key scenario at the M3(c) request count.
...
- [x] TB1 controls from [docs/zerosum_ledger_mvp_plan.md#trust-boundaries](zerosum_ledger_mvp_plan.md#trust-boundaries) are tested: constant-time token check, role per endpoint, body cap, unknown and duplicate fields rejected, source system derived from the principal. `libs/auth` also protects ledger-service read APIs with 401/403 (§0.3 C9).
- [x] Every master mapping row in [docs/zerosum_ledger_mvp_plan.md#event-to-order](zerosum_ledger_mvp_plan.md#event-to-order) (including the v1.2 no-order rows, §0.3 C5) has a unit test ([docs/zerosum_ledger_mvp_plan.md#test-layers](zerosum_ledger_mvp_plan.md#test-layers) unit layer).
- [x] The consumer creates exactly one order per `event_id` under repeated delivery, preserving the I8 convention ([docs/zerosum_ledger_mvp_plan.md#invariants](zerosum_ledger_mvp_plan.md#invariants)).
- [x] Every stored, fetched and published money order, and every consumed payment event, validates against the D01-8 schemas (contract layer). The published form is asserted on the mapper path in `PaymentEventConsumerIT`, so this does not rest on the API path alone.
...
| D03-6 | **Mapper** `services/order-service/src/main/java/dev/zerosum/order/mapping/PaymentEventMapper.java`: a pure function from a schema-valid event to `OrderDraft`, `NoOrder` or `Poison`. No clock, no I/O, no Spring — the effective time is the event's `occurred_at`, so replaying an event later produces the identical order. Amounts arrive as a positive magnitude and the sign convention is applied here (ADR-0003), two or three entries per row, zero-sum by construction. **No-order rows (§0.3 C5):** `CHARGE_DECLINED`, `REFUND_FAILED`, `PAYOUT_REJECTED` — each leaves the receivable or payable owed, so an order would invent a movement; each increments its own counter. **`PAYOUT_FAILED`** is emitted only after `PAYOUT_ACCEPTED` and maps to the reversal, while a submission decline arrives as `PAYOUT_REJECTED`, so the mapper needs no history to be correct. **Metadata:** `report_id` for settlement and `return_code` for `PAYOUT_RETURNED`, both matching the D01-9 goldens exactly; `PAYOUT_FAILED` has **no golden order** (the manifest records it as unpaired), so `failure_code` is a choice recorded here rather than one the goldens dictated. **Settlement (§0.3 C6, E1):** `net + fee = gross` is checked before mapping and a mismatch is poison — the report's totals are booked as reported, and a residual on provider clearing is an I9 reconciliation break, never a mapper adjustment. **Unknown type is poison**, never an ignored record, so a producer shipping a new event type is discovered immediately. **Consumer** `…/consumer/PaymentEventListener.java`: one record at a time, schema-validated first, `event_id` as the idempotency key with the internal mapper principal (`instrument-service`, §0.3 C7), the order and its outbox row committed together, then a manual acknowledgement — a crash between commit and ack redelivers, replays and acknowledges. `KEY_REUSED` is treated as an integrity breach and quarantined, not rewritten. **Poison handling:** quarantined in order-service's own `quarantined_events` table (shape per D02-9), inserted do-nothing-on-conflict keyed by Kafka coordinates, and the partition keeps moving. **DLQ publishing is deliberately not implemented here:** it needs a producer, and constructing Spring's `DeadLetterPublishingRecoverer` in the consumer package would name `KafkaTemplate` and trip the M4(b) rule; the dead-letter naming is S04-T03's (D04-4) in any case. The quarantine row holds the payload, so nothing is lost by deferring it. **Grants:** `orders_app` gets SELECT, INSERT and UPDATE on the quarantine table but **not DELETE** — resolving a poison record sets `resolved_at`, and evidence is not erased by the runtime role | The mapper is where a wrong entry becomes a permanently wrong ledger, so it is pure, golden-checked and re-validated through the same rules as an API order | Mapping in each producing service (drifting copies of the chart of accounts); a clock-based effective time (replay would produce a different order); skipping unmappable events (loses a payment fact silently); adjusting settlement residuals in the mapper (hides reconciliation breaks); DLQ publishing from the consumer package (would defeat the M4(b) publish-path rule) | Accepted | 2026-09-16 |
| D03-7 | **`GET /v1/outbox/stats`**, reader role. Fields `unpublished_count` and `oldest_unpublished_age_seconds`; age from `created_at` with the **database** clock (§0.3 C12). **Empty outbox reports 0 and 0, not an absent field**, so S05's payout-run freshness sum is always defined. Count and age come from one statement so both describe the same instant, using the partial index on unpublished rows. The query lives in `libs/outbox` (`OutboxStatsQuery`) for instrument-service to reuse in S05. Failures fall through to the standard problem advice (503 when the database is unreachable) | A staleness signal is only useful if it stays truthful while the pipeline is behind, which is exactly when it is read | Two statements (count and age could describe different instants); the application clock (drift would flatter the number); omitting the age when empty (every caller would special-case it); a service-local query (instrument-service would duplicate SQL against a library-owned table) | Accepted | 2026-09-16 |
...
| Payment-event mapper | `services/order-service/src/main/java/…/mapping/` | `PaymentEventMapper.java` (pure; no clock, no I/O, no Spring) | D03-6 |
| Order-mapper consumer, provisional consumer and poison settings | `services/order-service/src/main/java/…/consumer/`, `application.yaml` | `…/consumer/{PaymentEventListener,QuarantineStore,ConsumerConfiguration}.java`; `application.yml` `spring.kafka.consumer.*`, `spring.kafka.listener.*`, `zs.consumer.*` | D03-6 |
| Order-service quarantine table migration (§0.3 C8, C25) | `services/order-service/src/main/resources/db/migration/` | `V5__quarantined_events.sql` (D02-9 shape; SELECT/INSERT/UPDATE to `orders_app`, DELETE withheld) | D03-1, D03-6 |
...
| ADR-0006 single writer of money orders | `docs/adr/0006-*.md` | [docs/adr/0006-order-service-is-the-single-writer.md](adr/0006-order-service-is-the-single-writer.md) | — |
...
| Outbox stats semantics and fail-closed | `OutboxStatsIT` | **Passed** (2 tests): an unauthenticated read is 401, a reader and a writer token both get 200, the response carries exactly the two documented fields, and creating two orders moves `unpublished_count` by exactly two — which also proves the create path writes its outbox row in the same transaction. Run with **no broker**, the only state where the endpoint matters. **Exposed a real defect:** the problem advice was bound to `MoneyOrderController`, so this endpoint returned 500 where it documented 401, and every problem code collapsed into `internal_error`. Now scoped by package | [docs/results/s03/s03-t06-outbox-stats.txt](results/s03/s03-t06-outbox-stats.txt) | 2026-09-16 |
| Mapping rows, goldens, generative drafts | `PaymentEventMapperTest` | **Passed** (13 tests): each of the six paired D01-9 goldens is reproduced exactly — type, reason, group, entries in order, metadata and idempotency key — checked against the golden rather than against my reading of the table. The three no-order rows return `NoOrder` with their counter; `PAYOUT_FAILED` (no golden exists) asserts the master row directly; a settlement whose totals disagree and an undefined event type are both poison; every draft passes the same `ZeroSumValidator` an API order does. **Gap: the seeded generative test (D01-10) is not written** — the validator check covers the existing rows, not randomised inputs | [docs/results/s03/s03-t07-mapper-consumer.txt](results/s03/s03-t07-mapper-consumer.txt) | 2026-09-16 |
| Consumer idempotency, poison path | `PaymentEventConsumerIT` | **Passed** (5 tests, real broker and database): three deliveries of one event leave exactly one order and one outbox row, held stable rather than sampled once, attributed to the internal mapper principal; the published payload validates against the D01-8 schema; a decline creates no order and is **not** quarantined; poison is quarantined and a good event behind it still processes; the same Kafka coordinates quarantine once; and `orders_app` cannot DELETE the evidence (42501). **DLQ publishing deferred** to S04-T03 — it needs a producer, which in the consumer package would trip the M4(b) rule | [docs/results/s03/s03-t07-mapper-consumer.txt](results/s03/s03-t07-mapper-consumer.txt) | 2026-09-16 |
...
| S03-T06 | Done | `libs/outbox/src/main/java/dev/zerosum/outbox/OutboxStatsQuery.java`, `services/order-service/src/main/java/dev/zerosum/order/api/{OutboxStatsController,ApiAuthorization}.java`, `…/api/ApiExceptionHandler.java` (advice scoped by package), `…/order/outbox/OrderOutboxConfiguration.java` (stats bean), `openapi/order-service.yaml` (`/v1/outbox/stats` + `OutboxStats` schema), `…/api/OutboxStatsIT.java` | [docs/results/s03/s03-t06-outbox-stats.txt](results/s03/s03-t06-outbox-stats.txt): 60 order-service tests green | None. The S03-T04 forcing assertion is discharged and inverted |
| S03-T07 | Done | `services/order-service/src/main/java/dev/zerosum/order/mapping/PaymentEventMapper.java`, `…/consumer/{PaymentEventListener,QuarantineStore,ConsumerConfiguration}.java`, `…/db/migration/V5__quarantined_events.sql`, `application.yml` (consumer settings; **kafka and jackson blocks relocated under `spring:`**), `…/test/java/dev/zerosum/order/{mapping/PaymentEventMapperTest,consumer/PaymentEventConsumerIT}.java`, [docs/adr/0006-order-service-is-the-single-writer.md](adr/0006-order-service-is-the-single-writer.md) | [docs/results/s03/s03-t07-mapper-consumer.txt](results/s03/s03-t07-mapper-consumer.txt): 18 new tests; 312 project-wide green | **Two gaps recorded, not worked around:** the D01-10 seeded generative mapper test is not written, and DLQ publishing is deferred to S04-T03 (quarantine holds the payload meanwhile). Four defects of mine are in the evidence note, including a misplaced YAML block that silently disabled the listener |
...
|---|---|---|---|---|
| `docs/zerosum_ledger_mvp_plan.md` (v1.2) | `1df78b3 2026-09-15` | 2026-09-15 (doc 1.1); recomputed 2026-09-16 at S03 start | All | Revalidated: unchanged since S02 consumed it, so every inherited master value still resolves to the same text |
| `docs/README.md` | `1df78b3 2026-09-15` | 2026-09-16 (S03 start) | All | No impact: the shared procedures (source of truth, conflict resolution, change detection, status legend) are unchanged |
| `docs/step_00_foundations.md` (register) | `b4de2cb 2026-09-15` | 2026-09-16 (S03 start) | All | Current. Last changed by **CR-S02-05**, which added the `study` test layer to D00-10; S03 inherits the three CI-run layers plus `studyTest`, which no CI job invokes |
| `docs/step_01_domain_contracts.md` (register) | `f0c9240 2026-09-15` | 2026-09-16 (S03 start) | S03-T01, S03-T02, S03-T03, S03-T05, S03-T07 | Current. Last changed by **CR-S02-04**, which added the clearing classification to D01-6; the validator, chart of accounts and currency allow-list S03 consumes are otherwise as S01 accepted them |
| `docs/step_02_ledger_core.md` (register) | `d7a62eb 2026-09-15` | 2026-09-16 (S03 start) | S03-T02, S03-T07 | Complete and merged to `main` in `9eaa59a`, G1 proceed. S03-T02 copies the D02-2 append-only pattern; S03-T07 consumes D02-3's apply entrypoint contract. All twelve D02 decisions are recorded, none Pending |
...
| `settings.gradle.kts` (module layout) | `1df78b3 2026-09-15` | 2026-09-16 (S03 start) | S03-T03, S03-T05 | Unchanged. `libs:outbox`, `libs:auth` and `services:order-service` are already declared by D00-2, so S03 adds code to existing module slots rather than changing the layout |
...
| `infra/postgres/init.sql` and order-service Flyway baseline | `init.sql` `1df78b3 2026-09-15`; `V1__baseline.sql` `1df78b3 2026-09-15` | 2026-09-16 (S03-T01) | S03-T01, S03-T02, S03-T05 | Consumed unchanged. The baseline is empty by design, so `V2__orders_schema.sql` is the first orders migration; the D00-4 roles `orders_owner` and `orders_app` are used exactly as S00 created them |
| `.env.example` | — | — | S03-T03 | — |
