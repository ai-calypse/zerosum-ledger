...
| Jackson version skew for `libs/contracts` consumers: the pinned validator 3.0.7 is built against Jackson 3.2.1 (resolved in `libs/contracts`), while services import the Boot 4.1.1 BOM, which manages Jackson 3.1.5 | limitation (downstream risk) | A service test classpath would run the validator on Jackson 3.1.5; binary compatibility is untested | S03-T05 producer contract tests run `ContractSchemas` inside order-service; if they fail, raise a change request to D00-1 (align versions) |
