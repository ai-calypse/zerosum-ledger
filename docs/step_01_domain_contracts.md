...
- [x] **M1(a):** `ValidatorGenerativeTest` generates the order count required by M1(a) ([docs/zerosum_ledger_mvp_plan.md#must-have](zerosum_ledger_mvp_plan.md#must-have)). It accepts every balanced order, rejects every unbalanced one with its labeled code, prints its seed, and a replay with that seed reproduces the digest.
- [x] **M1(b):** the 0-digit and 3-digit currencies named in M1(b) are validated against the checked-in table generated from the source cited in [docs/zerosum_ledger_mvp_plan.md#standards](zerosum_ledger_mvp_plan.md#standards). The table's source hash is recorded in D01-2, and no money code uses JDK currency data (Rule E).
- [x] **M1(c):**
...
- [x] **M1(d):** golden O1–O7 validate exactly and reproduce the balances in [docs/zerosum_ledger_mvp_plan.md#worked-example](zerosum_ledger_mvp_plan.md#worked-example), and the O8 variant passes. This is the M1(d) scope set by §0.3 O6, and it satisfies the worked-example item in [docs/zerosum_ledger_mvp_plan.md#release-checklist](zerosum_ledger_mvp_plan.md#release-checklist).
- [x] The stored money-order schema (§0.3 C1) and the payment-event schema exist. Event types match §0.3 C5, settlement events follow C6, and mapper-created goldens follow C7. Every golden payload validates, every negative fixture is rejected, and the required fields per type and the versioning rule are recorded in D01-8.
- [x] `FeeCalculator` reproduces the worked-example fee, and it equals O6's processing-fee entry. `FareSplitter` sums exactly, is sign-symmetric and doesn't drift across adjustments.
- [x] The zero-sum rule definition that invariant I1 checks ([docs/zerosum_ledger_mvp_plan.md#invariants](zerosum_ledger_mvp_plan.md#invariants)) is documented in D01-5, precisely enough for S06-T04 to mirror it in SQL.
...
|---|---|---|---|---|---|
| D01-1 | `record Money(long amountMinor, String currency)` in `dev.zerosum.money`, with `Money.of`, `plus`, `minus`, `negate` and `isZero`. The constructor rejects codes that aren't three uppercase ASCII letters or aren't in the ISO table (`IllegalArgumentException` naming the code). Arithmetic uses `Math.addExact`, `subtractExact` and `negateExact`. Mixing currencies throws `CurrencyMismatchException` (extends `IllegalArgumentException`, names both codes). There is no decimal, floating-point or string-parsing constructor and no major-unit formatting. **Overflow policy:** arithmetic never wraps or saturates; `ArithmeticException` propagates. The per-entry cap of 10¹² is a validation rule (D01-5), not a `Money` invariant, because balances accumulate beyond one entry's cap. | Exact integer minor units, per master §5.2; overflow must be loud; no `BigDecimal` in the value type | Saturating arithmetic (silently wrong money); `BigDecimal` amount (scale bugs, pulls decimals into every consumer); cap as an invariant (breaks balances) | Accepted | 2026-09-15 |
| D01-2 | **Source:** ISO 4217 List One (SIX), published 2026-01-01, retrieved 2026-09-15 from the URL in master §2.4, SHA-256 `838dfb991648cf36df939edd5fe3811737962b75a32252847d239cedd1e291c9`. **Format:** UTF-8 text; `#` header lines (source, publication date, SHA-256, script, skipped codes, skipped entries, D01-2 trace) then `CODE,DIGITS` sorted by code, LF. **Location:** `libs/money/src/main/resources/dev/zerosum/money/iso4217-minor-units.csv` (165 codes: 17 × 0 digits, 139 × 2, 7 × 3, 2 × 4). **Regeneration:** `java libs/money/scripts/GenerateMinorUnitTable.java <list-one.xml> <out.csv>` (JDK-only; DTDs and external entities refused); deterministic output. **Entries without numeric minor units** (N.A.: XAG, XAU, XBA, XBB, XBC, XBD, XDR, XPD, XPT, XSU, XTS, XUA, XXX) and 3 entries without a code are skipped and listed in the header. Codes with 4 digits (CLF, UYW) are kept; the allow-list decides use. The **raw XML is not committed** because its redistribution terms weren't confirmed; the hash proves provenance. | Master step risk: JDK currency data lags ISO; reviewable exclusions; byte-identical regeneration | `java.util.Currency` (lags amendments, R8-style drift); hand-typed table (forbidden); committing the XML (terms unconfirmed) | Accepted | 2026-09-15 |
| D01-3 | `FeeCalculator.fee(Money base, long rateBps, Money fixed)` = `round(base × bps / 10 000) + fixed`, with `RoundingMode.HALF_EVEN` passed explicitly (`divide(…, 0, HALF_EVEN)`) and `longValueExact()`. **Input domain:** base, rate and fixed ≥ 0 (fees are computed on captures and never refunded), and `fixed` in the base currency. `FeeCalculator.roundedShare(long, long bps)` is the signed HALF_EVEN share used by `FareSplitter`, so `BigDecimal` stays in one class. Constants `ROUNDING = HALF_EVEN`, `BPS_SCALE = 10 000`. Schedules are always parameters. | Banker's rounding per master §5.2; narrow domain is easier to reason about; out-of-range results throw | Floating-point percentages (Rule A); signed fees (no caller needs them); embedding the FakeCard schedule (owned by S05/S06) | Accepted | 2026-09-15 |
| D01-4 | `FareSplitter.split(Money fare, long commissionBps)` → `Split(driverShare, platformCommission)`, bps 0…10 000 inclusive. Commission = `roundedShare(fare, bps)`; driver = fare − commission, so **the rounding remainder goes to the driver** and the parts always sum to the fare. Negative fares are allowed; HALF_EVEN makes `split(−f) = −split(f)` (tested over a fare × bps grid in USD, JPY, KWD). `adjustment(oldFare, newFare, bps)` = `split(newFare) − split(oldFare)` component-wise, **never** `split(newFare − oldFare)`. After any number of adjustments the cumulative shares equal `split(finalFare)`, whereas splitting deltas can drift one minor unit per adjustment. Returns amounts only; no bug-injection hook (A4 belongs to S08). | Exact sums (rule 9, Uber "Entity Fares"); drift-free adjustments | Remainder to platform (arbitrary; driver-favoring is the conventional choice); splitting deltas (drifts) | Accepted | 2026-09-15 |
| D01-5 | **Library rules** (`ZeroSumValidator.validate(OrderCandidate)` → ordered `List<Violation>`) implement master §5.3 rules 1–5. Rules 6–9 (adjustment reference and group, source type by principal, idempotency, entity fares) need DB state, the principal or caller policy, so **S03 owns them** (D03-1, D03-3, D03-4). **Codes** (`Violation.Code`): rule 1 `ORDER_MISSING`, `TYPE_INVALID`, `REASON_INVALID`; rule 2 `ENTRY_COUNT_OUT_OF_RANGE`, `ENTRY_MISSING`, `AMOUNT_MISSING`, `AMOUNT_ZERO`, `AMOUNT_OUT_OF_RANGE`; rule 3 `ENTITY_ID_INVALID`, `ACCOUNT_NOT_ALLOWED`; rule 4 `CURRENCY_NOT_ALLOWED` (with currency); rule 5 `ZERO_SUM_VIOLATED`, `ZERO_SUM_OVERFLOW` (with currency). **Order:** rule, then entry index, then currency (nulls first); all violations returned. **Evaluation:** null order → `ORDER_MISSING` only; count checked first, and above 50 per-entry work and rule 5 are skipped; an invalid entity ID skips its account check; per-currency sums use `addExact`, and an overflowing currency reports `ZERO_SUM_OVERFLOW` instead of `ZERO_SUM_VIOLATED`; strings are length-bounded before anchored patterns. **Limits** (`ValidationLimits`): types {COMMERCE, COLLECTION, REFUND, DISBURSEMENT, SETTLEMENT}; reason `^[a-z_]+\.[a-z_]+$`, max length 64 (selected here; master sets none); entries 2…50; \|amount\| ≤ 10¹², ≠ 0; entity `^(rider\|driver\|platform\|provider):[A-Za-z0-9_-]{1,64}$`, max length 73. **Repeated entity/account/currency lines are allowed** (§0.3 C4). **Consumer rule sets:** order-service runs `RuleSet.FULL` (the `validate(order)` default). **Amended by CR-S02-02 (2026-09-15):** ledger-service runs `validate(order, RuleSet.LEDGER)`, which is only the ‡ rules: entity ID parse and allowed account for its kind (rule 3), and zero-sum per currency with overflow detection (rule 5). Missing entries and amounts are still reported so the check stays total. The originally accepted text had both services run the full set. **I1 zero-sum definition** (for S06-T04 SQL): for each order and each currency, the sum of `amount_minor` over its entries equals 0, computed without overflow. | One rule set so S03-accepted orders can't be quarantined by S02; total on untrusted input (TB1) | Throwing on the first violation (callers lose the full picture); narrower ledger profile (would need a change request from S02) | Accepted | 2026-09-15 |
| D01-6 | [ADR-0003](adr/0003-sign-convention.md): debit +, credit −, zero-sum per currency, one normal side per account code (DEBIT: receivable, cash, processing_fees, clearing; CREDIT: payable, revenue, payout_clearing). Allowed accounts per **kind**: rider → receivable; driver → payable; platform → revenue, cash, processing_fees; provider → clearing, payout_clearing. Presentation: signed value for debit-normal accounts, negated for credit-normal ones, so **contra balances present as negative amounts on the normal side**. Code: `ChartOfAccounts`, `EntityKind`, `NormalSide`. **CR-S02-04 (2026-09-15, accepted):** `ChartOfAccounts` also classifies clearing accounts — `isClearing(account)` and `clearingAccounts()` over `clearing` and `payout_clearing`. ADR-0003 already made a non-zero clearing balance after quiesce the I9 health signal, but the classification was never exposed, so S02-T05's non-zero clearing report (§0.3 C13) would have had to hard-code the codes in ledger-service. The owner exposes it instead; adding a provider account now changes one place. | Master §5.2 table; kind-level keys support SP4 sub-entities and new providers | See ADR-0003 alternatives (direction flag, two-account transfers, debit/credit columns, credit-positive, side-label presentation) | Accepted | 2026-09-15 |
| D01-7 | Allow-list `libs/money/src/main/resources/dev/zerosum/money/allowed-currencies.txt`: EUR, JPY, KWD, USD (master §5.3 rule 4). Loaded with the table by `CurrencyRules`, which refuses to load when an allowed code is absent from the table, malformed or duplicated. `isAllowed(code)`. **No runtime override in the MVP.** The money-order schema's `currency_code` enum mirrors it, and `MoneyOrderSchemaTest` asserts they're equal. | One checked-in list, verified against ISO | Environment-variable override (config drift between services); allowing the whole ISO table (4-digit currencies, funds) | Accepted | 2026-09-15 |
| D01-8 | **Dialect:** JSON Schema draft 2020-12, validated with `com.networknt:json-schema-validator` 3.0.7 (D00-1). **Files and IDs:** `libs/contracts/src/main/resources/schemas/zerosum.money_order.v1.schema.json` (`$id` `https://zerosum.dev/schemas/zerosum.money_order.v1.schema.json`) and `zerosum.payment_event.v1.schema.json`. `ContractSchemas` maps the `$id` prefix to `classpath:schemas`, so there is no network access. **Money order** describes the stored order, which is the published event (C1); there is no request schema (D03-2). `$defs` (`uuid`, `group_id`, `entity_id`, `account_code`, `currency_code`, `signed_minor_amount`, `timestamp`, `entry`) are reusable by S03's OpenAPI. `schema` is a const. Unknown properties are rejected at the top level, in `source` and in entries; `metadata` stays open. Rules 1–4 shapes mirror D01-5/D01-7 with `$comment` traces; `MoneyOrderSchemaTest` asserts equality with `ValidationLimits` and the allow-list. **Payment event** has the event-type enum exactly as in §0.3 C5. Required fields per type: **all** — `schema`, `event_id`, `event_type`, `order_group_id`, `provider`, `occurred_at`; **every non-settlement type** — `attempt_id`, `entity_id`, `money{currency, amount_minor>0}`; **CHARGE_\*, REFUND_\*** — `source_order_id` (forbidden on PAYOUT_\*); **CHARGE_SUCCEEDED, REFUND_SUCCEEDED, PAYOUT_ACCEPTED, PAYOUT_FAILED, PAYOUT_SETTLED, PAYOUT_RETURNED** — `provider_ref`; **CHARGE_DECLINED, REFUND_FAILED, PAYOUT_REJECTED, PAYOUT_FAILED, PAYOUT_RETURNED** — `failure_code` (forbidden on successful types); **SETTLEMENT_RECEIVED** (C6) — `report_id`, `currency`, `gross_minor`, `fee_minor`, `net_minor`, with attempt, entity, source-order, provider-ref, failure and money fields forbidden. **Cross-field rules** that JSON Schema can't express (`event_id = <attempt_id>:<event_type>`; settlement `event_id = settlement:<provider>:<report_id>:<currency> = order_group_id`; `net + fee = gross`) are asserted by producer contract tests (D05-5, D06-3) and by `PaymentEventSchemaTest` on the goldens. **Integers:** 2020-12 accepts `2500.0` as an integer (documented by a test), so consumers' JSON parsers must reject non-integer lexical amounts (S02, S03). **Versioning:** the major version is in both the `$id` and the file name. Before v1.0.0, any v1 change goes through a change request to D01-8 and updates goldens in the same commit, with consumers marked "Needs review". After release, breaking changes (remove or rename a property, change a type, add a required property, narrow an enum or pattern) create a v2 file side by side. **Adding an optional property to v1 is a breaking change for strict consumers** because of `additionalProperties: false`, so after release it also needs v2. Topic suffixes belong to D04-1. | Strict contracts catch drift at the producer; one validator library pinned centrally; schema ↔ library equality tested | Draft 7 (weaker conditionals); Protobuf/Avro (deferred); a request schema here (C1 assigns it to S03); allowing unknown properties (silent drift) | Accepted | 2026-09-15 |
| D01-9 | **Fixture identities:** order IDs `01996a3e-8f10-7c2a-9a1e-3d5c7b2e4f0N` (N = 1…8; O1 is the master example ID). Groups: `trip_8f2c` (O1–O4), `payout_d1_0001` (O5, O7, O8), `settlement:fakecard:rpt_2026_09_15:USD` (O6 = its event's `event_id`, C6). Attempts `01996a3f-0001-7000-8000-00000000000N`: 1 charge, 2 refund, 3 payout (accepted, settled, returned), 4–7 schema-only events. Provider refs `ch_0001`, `re_0001`, `po_0001`. Report `rpt_2026_09_15`. Timestamps increase O1 (2026-09-15T10:04:11.219Z) → O8 (2026-09-18). O3 `adjusts_order_id` = O1, same group. **Writer `source.system` for O1/O3:** `trip-simulator`, the master example value, flagged in J for S03/D03-4 to adopt or change. **Mapper orders** (O2, O4–O8): `source.system = instrument-service`, `idempotency_key = event_id` (C7). **Files:** `golden/orders/o1…o8.json`; `golden/events/ev-o2, ev-o4, ev-o5, ev-o6, ev-o7, ev-o8.json` (paired) plus `ev-charge_declined, ev-refund_failed, ev-payout_rejected, ev-payout_failed.json` (schema-only; PAYOUT_FAILED has a mapping row but no golden order); `golden/expected-balances.json` (after O1–O7 and the O8 variant, signed and presented, O6 scope per §0.3 O6); `golden/manifest.json` (files, pairings, `write_time_generated_fields` = `order_id`, `created_at` for orders). **Loader:** `GoldenPayloads.byId("O6")` / `byId("EV-O6")` (file name = lower-case ID), `manifest()`, `expectedBalances()`. **Negative fixtures:** `libs/contracts/src/test/resources/negative/{money-order,payment-event}.json`, 15 + 12 patch cases, each with its expected failing keyword. | Deterministic, readable IDs; every C5 type has a positive fixture; the manifest makes ignored fields explicit | Random IDs per run (non-deterministic goldens); one file per negative case (noisier, same coverage) | Accepted | 2026-09-15 |
| D01-10 | **Location:** Gradle `java-test-fixtures` source set of `libs/money`, i.e. `libs/money/src/testFixtures/java/dev/zerosum/money/generate/` (§0.3 C16). Consumers use `testImplementation(testFixtures(project(":libs:money")))`; fixtures never ship in the runtime jar. Fixtures expose JUnit through `testFixturesApi(platform(spring-boot-dependencies))` + `junit-jupiter`. **RNG:** `RandomGeneratorFactory.of("L64X128MixRandom").create(seed)`, named once in `SeededExtension.ALGORITHM`; never `getDefault()`, `Math.random` or `ThreadLocalRandom`; draws only from ordered collections (allowed accounts are sorted before use). **Seed:** `SeededExtension` reads `ZS_TEST_SEED` (D00-8 naming; indexed as an optional S01 entry in `.env.example`), else `SecureRandom`; prints `ZS-SEED test=<class>#<method> seed=<long>`; on failure appends the seed line and `replay: ZS_TEST_SEED=<seed> ./gradlew <task> --tests '<class>.<method>' --rerun` (`libs/money` passes its task path as `zs.gradleTestTask`). **Generators:** `OrderGenerator.valid()` (1–3 allowed currencies; per currency 2+ lines with the last closing the sum; \|amount\| ≤ cap/64 so no overflow; ~10% repeated entity/account/currency lines; redraws ≤ 100 times, then throws with the seed) and `invalid()` (one of nine mutation families `OrderGenerator.FAMILIES`, labeled by the mutation and confirmed by literal-data self-checks, never by the validator); `TripSequenceGenerator.trips(rng, count, commissionBps, adjustmentPercent, currency)` (COMMERCE orders from `FareSplitter.split`, adjustments from `FareSplitter.adjustment`, zero lines omitted, integer percentages only); `StreamPerturbation.perturb(items, rng, duplicatePercent, maxDisplacement)` (bounded reordering by keyed stable sort). **Tests:** untagged, run by the `build` job (D00-10, D00-5). Details in [ADR-0009](adr/0009-seeded-generative-tests.md). | Deterministic replay across machines and JDK updates; oracle independent of the code under test; no floating point in money code | jqwik (maintenance status, anti-AI clause, JUnit Platform 1.x); junit-quickcheck (unmaintained); generators in `src/test` (not reusable by S02/S08) | Accepted | 2026-09-15 |
| D01-11 | `MoneyArchRules` in `libs/money/src/testFixtures/java/dev/zerosum/money/arch/`, pinned ArchUnit 1.5.0 via `testFixturesApi` (its shaded ASM reads class files up to major 71, so Java 25 is supported; R8 cleared). **Every rule is a factory taking a scope predicate** (`noFloatingPoint`, `bigDecimalOnlyInFeeCalculator`, `explicitHalfEven`, `jdkOnly(scope, ownPackages…)`, `noJdkCurrencyData`), so services apply the same rules to their money-handling packages; `forLibsMoney()` returns the five with the libs/money scopes. **Money scope:** `dev.zerosum.money` (main) plus `dev.zerosum.money.generate..` (test fixtures). The `arch` package is excluded as tooling, and test classes including `dev.zerosum.archcanary` are excluded by `ImportOption.DO_NOT_INCLUDE_TESTS`. **Rule A:** no `float`, `double`, `Float` or `Double` in fields, code-unit signatures, code-unit calls, method/constructor references or field accesses. **Rule B allow-list:** `dev.zerosum.money.FeeCalculator` only. **Rule C:** no `RoundingMode` constant other than `HALF_EVEN`; no dependency on `MathContext`; no `BigDecimal.divide(BigDecimal)`, `divide(BigDecimal,int)`, `divide(BigDecimal,int,int)`, `setScale(int)` or `setScale(int,int)` (implicit or legacy-int rounding). **Rule D scope:** main classes (`dev.zerosum.money`, no subpackages) depend only on `java..` and `dev.zerosum.money`; test fixtures may use JUnit and ArchUnit. **Rule E:** no dependency on `java.util.Currency`. **Canaries:** one per rule in `libs/money/src/test/java/dev/zerosum/archcanary/`, each imported explicitly by `MoneyArchitectureTest`, which asserts its rule reports it. **Blind spot:** primitive floating-point locals and arithmetic opcodes aren't visible (H.5). Outbox (S03) and provider (S05) rules aren't part of S01 (C16). | M1(c) enforced at build time; reusable by services without copying rules | `@AnalyzeClasses` JUnit engine rules (no per-rule canary evaluation); fixed scopes (not reusable by services); scanning the `arch` package (flags its own `RoundingMode`/`BigDecimal` literals) | Accepted | 2026-09-15 |

### H.2 Implementation and configuration locations

| Item | Planned path | Actual path | Traced to |
|---|---|---|---|
| `Money`, `CurrencyMismatchException`, `CurrencyRules` | `libs/money/src/main/java/<root>/money/` | `libs/money/src/main/java/dev/zerosum/money/{Money,CurrencyMismatchException,CurrencyRules}.java` | D01-1 |
| ISO 4217 minor-unit table | `libs/money/src/main/resources/<root>/money/iso4217-minor-units.csv` | `libs/money/src/main/resources/dev/zerosum/money/iso4217-minor-units.csv` | D01-2 |
| Table regeneration script | `libs/money/scripts/GenerateMinorUnitTable.java` | same | D01-2 |
| `FeeCalculator` (rounding mode, bps scale) | `libs/money/src/main/java/<root>/money/FeeCalculator.java` | `libs/money/src/main/java/dev/zerosum/money/FeeCalculator.java` (`ROUNDING`, `BPS_SCALE`) | D01-3 |
| `FareSplitter` | `libs/money/src/main/java/<root>/money/FareSplitter.java` | `libs/money/src/main/java/dev/zerosum/money/FareSplitter.java` | D01-4 |
| `ZeroSumValidator`, violation codes, `ValidationLimits` | `libs/money/src/main/java/<root>/money/` | `libs/money/src/main/java/dev/zerosum/money/{ZeroSumValidator,Violation,OrderCandidate,ValidationLimits}.java` | D01-5 |
| `ChartOfAccounts`, `EntityKind`, `NormalSide` | `libs/money/src/main/java/<root>/money/` | `libs/money/src/main/java/dev/zerosum/money/{ChartOfAccounts,EntityKind,NormalSide}.java` | D01-6 |
| Currency allow-list | `libs/money/src/main/resources/<root>/money/allowed-currencies.txt` | `libs/money/src/main/resources/dev/zerosum/money/allowed-currencies.txt` | D01-7 |
| JSON Schemas | `libs/contracts/src/main/resources/schemas/` | `libs/contracts/src/main/resources/schemas/zerosum.{money_order,payment_event}.v1.schema.json`; loader `libs/contracts/src/main/java/dev/zerosum/contracts/ContractSchemas.java` | D01-8 |
| Golden payloads, expected balances, manifest | `libs/contracts/src/main/resources/golden/` | `libs/contracts/src/main/resources/golden/{orders,events}/*.json`, `expected-balances.json`, `manifest.json`; loader `…/contracts/GoldenPayloads.java`; negatives `libs/contracts/src/test/resources/negative/` | D01-9 |
| Generators, seed extension, RNG algorithm constant, seed variable name | `libs/money/src/testFixtures/java/<root>/money/generate/` | `libs/money/src/testFixtures/java/dev/zerosum/money/generate/{Seed,SeededExtension,OrderGenerator,TripSequenceGenerator,StreamPerturbation}.java`; `ZS_TEST_SEED` indexed in `.env.example` | D01-10 |
| `MoneyArchRules` (scope, `BigDecimal` allow-list) | `libs/money/src/testFixtures/java/<root>/money/arch/` | `libs/money/src/testFixtures/java/dev/zerosum/money/arch/MoneyArchRules.java` | D01-11 |
...
|---|---|---|---|---|
| Money arithmetic, overflow, currency validation | `MoneyTest`, `CurrencyRulesTest` | **Passed**: 13 + 8 tests (exact arithmetic, overflow at both `long` bounds, mismatch, rejected code shapes, table digits USD 2 / JPY 0 / KWD 3, sorted without duplicates, malformed resources named) | [s01-t01-money-table.txt](results/s01/s01-t01-money-table.txt) | 2026-09-15 |
| ISO table regeneration reproduces checked-in file | Script run + `diff` against recorded source hash | **Passed**: `diff` prints nothing; header SHA-256 equals the input file hash; a DOCTYPE/external-entity input is refused with exit 1 and no output; `grep -rn java.util.Currency libs/money/src/main` finds nothing | [s01-t01-money-table.txt](results/s01/s01-t01-money-table.txt) | 2026-09-15 |
| Fee rounding and worked-example fee | `FeeCalculatorTest` | **Passed**: 12 tests; fee(2500, 290 bps, 30) = 102; ties round to even in both signs; domain rejections; out-of-range throws | [s01-t02-fee-split.txt](results/s01/s01-t02-fee-split.txt) | 2026-09-15 |
| Fare split exactness, symmetry, adjustment drift | `FareSplitterTest` | **Passed**: 4 tests; O1 (2000/500) and O3 (−240/−60) reproduced; sum and sign symmetry over a fare × bps grid in USD/JPY/KWD; cumulative adjustments equal `split(finalFare)` | [s01-t02-fee-split.txt](results/s01/s01-t02-fee-split.txt) | 2026-09-15 |
| Chart of accounts and validator rules 1–5 | `ChartOfAccountsTest`, `ZeroSumValidatorTest` | **Passed**: 20 + 13 tests; every chart row, disallowed pair, sub-entity, contra presentation; accept and reject per rule 1–5, ordering, null-safety, overflow as violation, count guard, unknown allow-list currency fails to load, repeated lines (C4), O1–O8 in code | [s01-t03-validator-chart.txt](results/s01/s01-t03-validator-chart.txt) | 2026-09-15 |
| ADR-0003 accepted and linked | File review, path checks | **Passed**: status Accepted; `test -f` on all 4 linked paths; contra-balance test named in the ADR passes. No ADR index exists yet (S09-T02) | [s01-t04-adr-0003.txt](results/s01/s01-t04-adr-0003.txt) | 2026-09-15 |
| Schemas accept goldens, reject negatives | `MoneyOrderSchemaTest`, `PaymentEventSchemaTest` | **Passed**: 8 orders and 10 events valid; 15 + 12 negative cases each rejected with the expected keyword (`required`, `additionalProperties`, `enum`, `const`, `pattern`, `minItems`, `not`, `type`, `maximum`, `exclusiveMinimum`, `false`); the event-type enum equals the C5 list, and every type has a positive fixture; C5/C6 `event_id` and net + fee = gross rules hold; schema limits equal `ValidationLimits`; `2500.0` documented as accepted | [s01-t05-contracts.txt](results/s01/s01-t05-contracts.txt) | 2026-09-15 |
| Worked example O1–O7 plus the O8 variant, and balances (M1(d), §0.3 O6) | `WorkedExampleGoldenTest`, `GoldenManifestTest` | **Passed**: 5 + 2 tests; all goldens pass the validator and sum to 0 per currency; balances after O1–O7 (revenue −440, fees 102, cash 338, others 0) and after O8 (cash 2098, payable −1760) match signed and presented; `FeeCalculator` on O2's capture = O6 fee 102; paired events map to their orders (C7 sources); manifest lists exactly the golden files | [s01-t05-contracts.txt](results/s01/s01-t05-contracts.txt) | 2026-09-15 |
| M1(a) bulk validator test with seed | `ValidatorGenerativeTest` | **Passed**: 10,000 orders (5,000 valid accepted; 5,000 invalid each carrying its label, all 9 families exercised); 500 valid + expressible invalid samples agree with the schema; generated trips validate and accumulate to a direct split; seed lines printed | [s01-t06-generative.txt](results/s01/s01-t06-generative.txt) | 2026-09-15 |
| Seed determinism and replay | `GeneratorDeterminismTest` + replay run | **Passed**: `ZS-DIGEST seed=8119415789160221247 sha256=19f9f818…78a0` identical on replay with `ZS_TEST_SEED`; a different seed gives a different digest; perturbation displacement ≤ bound | [s01-t06-generative.txt](results/s01/s01-t06-generative.txt) | 2026-09-15 |
...
| Jackson version skew for `libs/contracts` consumers. The pinned validator 3.0.7 needs Jackson 3.2.1, while the Boot 4.1.1 BOM manages 3.1.5. **Corrected 2026-09-15 by an S02 probe:** Gradle treats the BOM platform as a constraint, so conflict resolution selects **3.2.1** in services too (`dependencyInsight` on ledger-service), and `ContractSchemas` validates goldens there. The earlier assumption that services would pin 3.1.5 was wrong. | limitation (downstream risk) | Any Boot service that depends on `libs/contracts` runs Spring Boot 4.1.1 on Jackson 3.2.1, a version Boot doesn't manage | Recorded for D00-1 in S02 I.2 (CR-S02-03). Spring-context integration tests in S02-T04 and later exercise Boot on 3.2.1; if they fail, raise a change request to D00-1 |
