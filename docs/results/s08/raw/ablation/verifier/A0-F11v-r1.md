# A0-F11v-r1 — invariants I1, I2, I3, I4, I6, I6b, I7
...
Ledger scope actually examined: **entities=251 accounts=251 changelog_rows=17835 applied_orders=5945**. Each database was read inside one read-only repeatable-read
...
```sh
./gradlew :tools:verifier:run --args="--jdbc-url jdbc:postgresql://127.0.0.1:5432/ledger --orders-jdbc-url jdbc:postgresql://127.0.0.1:5432/orders --instruments-jdbc-url jdbc:postgresql://127.0.0.1:5432/instruments --providers-jdbc-url jdbc:postgresql://127.0.0.1:5432/fakeproviders --checks I1,I2,I3,I4,I6,I6b,I7 --env-file /Users/yakshgandhi/Documents/Development/fintech-blogs/zerosum-ledger/.env --out /Users/yakshgandhi/Documents/Development/fintech-blogs/zerosum-ledger/infra/tests/build/evidence/ablation/verifier --label A0-F11v-r1"
```

## 7. Raw data

[A0-F11v-r1.json](A0-F11v-r1.json) — the same run, with every check's metrics and its sample of offending rows.

## 8. Results

| Invariant | What it checks | Result | Violations | Metrics |
|---|---|---|---|---|
| I1 | every money order is zero-sum per currency | **PASS** | 0 | orders_checked=5945, unbalanced_orders=0 |
...
| I6 | order ids in the orders DB = applied_orders, and 0 unresolved quarantine rows | **PASS** | 0 | orders_in_store=5945, applied_orders=5945, missing_orders=0, extra_applied=0, unresolved_quarantined=0 |
