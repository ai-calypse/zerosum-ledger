# m11c-run4-verifier — invariants I1, I2, I3, I4, I6, I6b, I7, I10, I12
...
| Field | Value |
|---|---|
| Evidence ID | m11c-run4-verifier |
| Type | verification |
...
After M11(c) run rc920dcb45 (m11c-run4), both settlement cycles quiesced.
...
Ledger scope actually examined: **entities=206 accounts=208 changelog_rows=12024 applied_orders=4808**. Each database was read inside one read-only repeatable-read
...
| I6 | order ids in the orders DB = applied_orders, and 0 unresolved quarantine rows | **PASS** | 0 | orders_in_store=4808, applied_orders=4808, missing_orders=0, extra_applied=0, unresolved_quarantined=0 |
| I6b | every ledger balance = Σ entries for that account across all orders in the order store | **PASS** | 0 | accounts_compared=208, accounts_drifting=0, drift_abs_minor=0 |
| I7 | provider ground truth ↔ attempts: one provider success per attempt, none unclaimed | **PASS** | 0 | attempts=2400, provider_success_records=2400, duplicate_charges=0, duplicate_minor=0, success_without_provider_record=0, provider_success_without_attempt=0, provider_success_on_failed_attempt=0 |
| I10 | 0 attempts in SUBMITTING/UNKNOWN older than 5 min; 0 NEEDS_REVIEW | **PASS** | 0 | stuck_over_5_min=0, needs_review=0 |
| I12 | every break matches an injected report discrepancy, and every injection has its break | **PASS** | 0 | reconciliation_runs=8, breaks=343, injected_discrepancies=343, explained_breaks=343, unexplained_breaks=0, undetected_injections=0 |
