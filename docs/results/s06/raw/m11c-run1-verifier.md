# m11c-run1-verifier — invariants I1, I2, I3, I4, I6, I6b, I7, I10, I12
...
| Field | Value |
|---|---|
| Evidence ID | m11c-run1-verifier |
| Type | verification |
...
After M11(c) run rcb19452f7 (m11c-run1), both settlement cycles quiesced.
...
Ledger scope actually examined: **entities=53 accounts=55 changelog_rows=3006 applied_orders=1202**. Each database was read inside one read-only repeatable-read
...
| I6 | order ids in the orders DB = applied_orders, and 0 unresolved quarantine rows | **PASS** | 0 | orders_in_store=1202, applied_orders=1202, missing_orders=0, extra_applied=0, unresolved_quarantined=0 |
| I6b | every ledger balance = Σ entries for that account across all orders in the order store | **PASS** | 0 | accounts_compared=55, accounts_drifting=0, drift_abs_minor=0 |
| I7 | provider ground truth ↔ attempts: one provider success per attempt, none unclaimed | **PASS** | 0 | attempts=600, provider_success_records=600, duplicate_charges=0, duplicate_minor=0, success_without_provider_record=0, provider_success_without_attempt=0, provider_success_on_failed_attempt=0 |
| I10 | 0 attempts in SUBMITTING/UNKNOWN older than 5 min; 0 NEEDS_REVIEW | **PASS** | 0 | stuck_over_5_min=0, needs_review=0 |
| I12 | every break matches an injected report discrepancy, and every injection has its break | **PASS** | 0 | reconciliation_runs=2, breaks=83, injected_discrepancies=83, explained_breaks=83, unexplained_breaks=0, undetected_injections=0 |
