...
| Trace propagation across the pipeline (M12(a)) | Live stack, spans read from Tempo | **Broken at the outbox hop — M12(a) is NOT met, and no claim is made that it is.** Three traces where one is required: `POST /v1/money-orders` (`adb50356bf69f536b62819dd700eab0c`, no producer span at all), `payments.money-orders.v1 publish` (`6d95da62c5376119042c545083a5f9e4`, an **orphan root** — one span, parented by nothing), and `payments.money-orders.v1 process` (`c35834254ea0c9c0428779a4146c2ca6`, root with **1 link** to the publish trace). The Kafka hop is therefore fine and behaves as links, not parent-child; the break is that the relay's send starts its own trace instead of continuing the request's. **Cause:** `OutboxWriter` stores `traceparent` and `OutboxRelay` copies it onto the record, but the agent builds the publish span from the relay thread's (empty) context and overwrites the header — storing a header is not restoring a context. **Not patched from S04:** the relay is S03's, so **CR-S04-01** is raised against D03-5 | [docs/results/s04-trace-propagation.md](results/s04-trace-propagation.md) | 2026-09-16 |
...
| S04-T06 | Done | [docs/results/s04-trace-propagation.md](results/s04-trace-propagation.md) | Three trace IDs recorded from the live stack | **Result is a defect, not a pass:** broken at the outbox hop, so M12(a) is not met. **CR-S04-01** raised to S03 against D03-5. The consumer hop is links rather than parent-child, which S07-T01 must account for |
