...
- [x] **M12 (a) input:** the single trace from API to ledger apply is verified (§0.3 C11) and recorded in `docs/results/s04-trace-propagation.md`, and any gap has a change request or handoff item. *(Verified on the live stack, and the defect it found was fixed and re-verified. **Connected (links):** the publish span is now a child of the request trace and the ledger's process span links into it. CR-S04-01 is implemented. The consumer hop remains links rather than parent-child — handed to S07-T01, not a defect.)*
...
| D04-3 | **Batch listener, manual acknowledgement after the database commit.** `services/ledger-service/src/main/java/dev/zerosum/ledger/kafka/MoneyOrderListener.java` takes the whole poll batch (`spring.kafka.listener.type: batch`) and hands it to the D02-3 entrypoint in **one** `engine.apply(List)` call, preserving offset order within each partition; cross-partition order is irrelevant because a group is confined to one partition (ADR-0007) and balance arithmetic is commutative. **Acknowledgement is `MANUAL` (queued), once per poll batch, and only on the line after `apply()` returns** — which means after its transaction committed. An exception propagates instead, leaving offsets untouched so the batch is redelivered and deduped. `MANUAL_IMMEDIATE` was not chosen: the commit timing gains nothing here and the queued form keeps one commit per poll. **Each record carries its source position** (topic, partition, offset) into `ApplyRecord.SourcePosition`, which is what lets the engine key a quarantine row to its origin and what makes a redelivered poison record land on the existing row. **An empty poll opens no transaction and acknowledges nothing.** **`ledger.consumer.enabled`** gates `autoStartup`, so the many ledger tests that have no broker do not start a listener dialling localhost. **Decoding stays in the engine:** values are consumed as `String`, so malformed JSON is not a Kafka deserialization failure but a decode failure the engine already quarantines with its own error code — no `ErrorHandlingDeserializer` is configured, and that is a choice, not an omission. **Not implemented (deferred, [docs/scope-decisions.md](scope-decisions.md)):** listener concurrency is left at the container default of 1 rather than tuned against the Hikari pool; the D04-8 crash-point seam; `GracefulShutdownIT`; and counting an offset-commit failure that follows a successful database commit | Acknowledging after the commit is the only ordering that cannot lose money: the inverse skips orders that never applied, while this one can only duplicate, which the engine's `applied_orders` dedupe absorbs | `MANUAL_IMMEDIATE` (more commits, no benefit at this batch size); per-record engine calls (loses the batch the engine was built to take, and multiplies transactions); auto-commit (an offset could advance past an unapplied order); acknowledging inside the transaction (a rollback would leave the offset moved); an `ErrorHandlingDeserializer` (would split decode failures across two quarantine paths) | Accepted | 2026-09-16 |
| D04-4 | **Classification.** *Transient* is exactly D02-4's classifier set (40P01, 40001, 55P03, 57P01, 57P03 and all of class 08). *Poison* is what the engine already reports as a `QuarantineCode` (undecodable payload, schema invalid, structurally invalid, arithmetic overflow, unexpected database error for one record). *Unclassified* is everything else and is handled like exhausted transient failure — **pause, never dead-letter**, because a dead-lettered valid order is skipped money. **One retry layer only:** the apply engine already retries transient SQLSTATEs with jittered backoff and throws `RetriesExhaustedException`; the container error handler therefore adds **no** backoff and simply reacts to exhaustion. **Poison path driven by outcomes, not by a recoverer.** The engine isolates a bad record, writes its quarantine row inside the batch transaction and *returns* a `QUARANTINED` outcome rather than throwing — so Spring's `DeadLetterPublishingRecoverer` would never fire, since nothing reaches the error handler. `MoneyOrderListener` reads the outcomes and publishes to the DLQ itself, in the order **quarantine row → DLQ publish and broker confirmation → acknowledge**; a crash between the publish and the ack duplicates a DLQ record, which is tolerable, whereas the reverse would acknowledge a record recorded only in this service's database. **DLQ record** keeps the original key, value and headers (including trace context) and adds `zs-dlq-original-topic`, `-original-partition`, `-original-offset`, `-classification`, `-error-code` and `-error-detail`. **No explicit partition is set:** the DLQ has 3 partitions against the source's 12, and the stock recoverer's default of reusing the source partition number would address a partition that does not exist. **Pause** is whole-container (a database fault affects every partition), seeks back to the first offset of the failed batch so nothing is skipped, and raises a `ledger_listener_paused` gauge plus a `ledger_listener_pauses_total` counter and one WARN line carrying no payload. **Resume is unattended:** a scheduled probe (`ledger.consumer.resume-probe-interval`, 5 s) runs `SELECT 1` on the service's own pool — the very resource whose absence caused the pause — and resumes from the saved position on success. **Ledger-service may hold a Kafka producer** even though M4(b) forbids one in order-service: the rule constrains *producing* services, where a direct send could publish an order that never committed. Ledger-service publishes no orders, only dead-letter copies of records it has already quarantined, so the hazard does not arise — and order-service's own DLQ stays deferred for precisely the opposite reason. **Not implemented (deferred, [docs/scope-decisions.md](scope-decisions.md)):** a container-level `PauseResumeIT` under a real database fault, `DlqPartitioningIT`, `QuarantineIdempotencyIT`, `MapperPoisonIT` on the order-service side, the runbook dry run, and lengthening the probe interval up to a cap after repeated failures (it is a fixed 5 s) | Retrying in two layers would multiply the budget past the poll interval; dead-lettering anything that is not provably poison throws away money; and driving the DLQ from outcomes is the only design that actually publishes, given that the engine returns rather than throws | A second backoff in the container (nested retries, lost assignment); `DeadLetterPublishingRecoverer` (never fires here); dead-lettering unclassified failures (skips valid orders); pausing single partitions (a database fault is not partition-specific); a manual resume step (S08 expects unattended recovery) | Accepted | 2026-09-16 |
| D04-5 | **`GET /v1/freshness`**, reader role. Fields: `status` (`ok` or `error`), `total_lag_records`, `partition_lag` (per partition, so one stuck partition is visible rather than averaged away), `oldest_unapplied_age_seconds`, `listener_paused` (from the D04-4 pause signal, §0.3 O9), `computed_at`, and `error`. **Fails closed:** any admin or fetch failure, timeout, partition-count mismatch, or a committed offset below the log start returns `status: error` with the numbers **omitted rather than zeroed**. **Timestamp basis (§0.3 C12):** the Kafka record timestamp of the oldest unapplied record, while outbox ages come from their own `created_at`, so the composite pipeline freshness S05-T10 assembles counts no stage twice. **Lag** is read through the admin client (`listConsumerGroupOffsets` against `ledger-apply`, `listOffsets` for latest and earliest); a partition with no committed offset counts from its earliest available record, not from zero, which would count records retention has already removed. **Age** is read by a group-less consumer assigned directly to the lagging partitions, so measuring freshness never disturbs the apply group's offsets. A record timestamp in the future is clamped to zero rather than reported negative. **No cache.** Published additively in `openapi/ledger-service.yaml` with a D04-5 trace comment | A freshness endpoint exists to be trusted when things are wrong; reporting zero for an unknown value is worse than reporting nothing, because S05 refuses above 5 s and a false zero would authorise a payout against unchecked balances | Returning zero on failure (silently authorises payouts); computing age from the application clock (drift flatters the number); reusing the apply consumer group to read timestamps (would move its offsets); caching (the reported age would exclude the cache's own age) | Accepted | 2026-09-16 |
| D04-6 | **No container-image e2e harness is built, and the D04-6 suite is not delivered as specified.** The preferred option — order-service and ledger-service as containers from project images on one network with Testcontainers PostgreSQL and Kafka — is the same lift as the deferred M4(a) SIGKILL harness, and is deferred under [docs/scope-decisions.md](scope-decisions.md). **What stands in its place, cited rather than re-run:** Case A/B substance is covered at listener level by `LedgerListenerIT` (goldens O1–O7 through a real broker produce the D01-9 balances with I2–I4 clean) and `DuplicateDeliveryIT` (three interleaved deliveries leave one `applied_orders` row each and one delivery's changelog), with the API-to-relay half covered by S03's outbox evidence. **Case C is Blocked on the D04-8 crash seam**, which this step deferred. **Case D is Blocked on a `libs/outbox` injection point that does not exist in D03-5**; per the task's own instruction the library is *not* forked and a change request goes to S03 instead. **Case E (M6(c) audit walk) is the one case kept**, because it needs only the public read APIs. **Therefore M5(a)–(d) end to end, the M4(c) downstream half and M6(c) are not claimed from a `PipelineE2ETest`** — the `e2eTest` task still selects no tests, and the CI e2e job remains schedule/dispatch only | A suite that cannot exercise its two crash cases would prove less than the listener-level tests already do, while reading like full end-to-end coverage — the more dangerous of the two outcomes | Building the image harness (the deferred M4(a) lift, on a machine whose Docker VM is already tight); forking `libs/outbox` to add a crash seam (the task forbids it and it would put a failure switch in shipped code); writing Cases C and D against in-process contexts (a halted object is not a halted process, which is exactly the gap M4(a) already records) | Accepted | 2026-09-16 |
| D04-7 | **Not measured — SP2 is not triggered, and that is not the same as the criterion being met.** S04-T07's relay-lag measurement is deferred under [docs/scope-decisions.md](scope-decisions.md): it needs an open-model load driver and a quiet reference machine, and it answers a tuning question about a latency problem nobody has observed. **No p50 or p95 publish lag exists for this build.** S04-C01 (relay tuning and the Debezium Outbox Event Router spike) is therefore **not triggered**, because triggering it requires evidence that the criterion was missed, and there is no evidence in either direction. The question hands to [S07-T05](step_07_observability_performance.md), which owns performance measurement, and ADR-0008 already records Debezium as the documented upgrade path | A spike funded by contingency hours must be triggered by a measurement, not by a guess; and recording "not triggered" without saying why would read as though the relay had been shown fast enough | Running an informal single-repetition measurement anyway (a number nobody should act on is worse than no number); triggering SP2 pre-emptively (3 h of contingency spent on an unobserved problem); recording the criterion as met (it was never tested) | Accepted | 2026-09-16 |
| D04-8 | — | — | — | Pending | — |
...
|---|---|---|---|---|
| Topic provisioning and mismatch behavior | `TopicProvisioningIT`, `TopicMismatchIT` | **Partial.** `TopicProvisioningIT` **passed** (3 tests): every defined topic is created with its contracted partition count and retention, a second declaration of an existing topic raises exactly `TopicExistsException` and leaves the topic unaltered, and DLQ names are derived rather than spelled twice. **`TopicMismatchIT` and the startup partition-count check are not implemented** — deferred per [docs/scope-decisions.md](scope-decisions.md), so a leftover topic with a different partition count is still undetected. Not claimed as passing. **Amended again after S04-T06:** the claim that topics "are created from the D04-1 definitions" was **stronger than what this test proved**. `TopicProvisioningIT` creates topics through the admin client directly, so it exercised the definitions but never the provisioning path the services actually use. On the real stack no topic was created at all: the `NewTopic`s were declared as a `@Bean List<NewTopic>`, and Spring's `KafkaAdmin` collects beans *of type* `NewTopic`, so a bean of type `List` was invisible to it. The consumer sat on `UNKNOWN_TOPIC_OR_PARTITION` while the outbox backed up. Now declared as individual `NewTopic` beans. **Amended after S04-T03:** this test was then found to be timing-flaky — it passed in isolation and failed in a busy full run with `UnknownTopicOrPartitionException`, because `createTopics().get()` returns when the controller accepts the creation rather than when metadata has propagated to the broker answering the describe. It now waits on topic visibility with a deadline. The original pass was real but not reliably reproducible | [docs/results/s04/s04-t01-topics-clients.txt](results/s04/s04-t01-topics-clients.txt) | 2026-09-16 |
| Explicit client configuration and group protocol | Per-service configuration tests; group type check | **Partial.** `KafkaClientConfigTest` **passed** (3 tests) against the *effective* built properties: consumer group, `auto.offset.reset`, auto-commit off, `read_committed`, `max.poll.records=500`; producer `acks=all` with idempotence; and `ack-mode=MANUAL` with `type=BATCH`. **The KIP-848 group-protocol assertion after first poll is not implemented** (deferred). Order-service's equivalent configuration test is not yet written. **Gap exposed on the real stack (S04-T06):** this test reads the properties Spring *built*, which is not the same as the address a deployed container *reaches*. Both services shipped with `bootstrap-servers` defaulting to `localhost:9092` and compose never set `ZS_KAFKA_BOOTSTRAP`, so inside their containers they dialled themselves — no topics, no consumer group, and the outbox backing up — while every test stayed green because Testcontainers always injects a real address. Nothing in the suite asserts the deployed wiring; only running the stack found it | [docs/results/s04/s04-t01-topics-clients.txt](results/s04/s04-t01-topics-clients.txt) | 2026-09-16 |
| Partition key | `PartitionKeyIT` | **Passed** (2 tests): 20 records for one order group land on exactly one partition, and 60 distinct groups spread across more than one without exceeding the contracted count. Carried over from T01, where it could not run because nothing published yet | [docs/results/s04/s04-t02-listener.txt](results/s04/s04-t02-listener.txt) | 2026-09-16 |
| Ack only after commit | `AckOrderingTest` | **Passed** (3 tests, unit): an engine double that throws is never acknowledged; one that returns is acknowledged exactly once per poll batch, with the acknowledgement itself asserting the engine had already returned; an empty poll opens no transaction and acknowledges nothing. Kept as a unit test deliberately — the claim is about call order, and a container would let it pass on timing | [docs/results/s04/s04-t02-listener.txt](results/s04/s04-t02-listener.txt) | 2026-09-16 |
| Listener correctness under duplicates | `LedgerListenerIT`, `DuplicateDeliveryIT` | **Passed** (3 tests, real broker and database). Goldens O1–O7 published by order group produce the D01-9 `after_O1_O7` balances exactly, with I2, I3 and I4 clean on state that arrived through Kafka; every applied order records the topic, partition and offset it came from. **M5(a):** three interleaved deliveries of every order leave 7 `applied_orders` rows, identical balances, and a changelog holding exactly one delivery's entries — derived from the goldens rather than hard-coded, since balances alone could look right while the changelog grew | [docs/results/s04/s04-t02-listener.txt](results/s04/s04-t02-listener.txt) | 2026-09-16 |
| Graceful shutdown | `GracefulShutdownIT` | **Deferred, not run.** Dropped with the D04-8 crash-point seam under [docs/scope-decisions.md](scope-decisions.md); the in-flight-batch-on-shutdown behaviour is therefore unproven, and is not claimed | — | — |
| Poison → quarantine + DLQ | `PoisonRecordIT`, `DlqPartitioningIT`, `QuarantineIdempotencyIT` | **Partial.** `PoisonRecordIT` **passed** (2 tests): a valid order, undecodable bytes and another valid order on one partition — both valid orders present, the undecodable record quarantined with its Kafka coordinates, and the DLQ record carrying the original key plus the `zs-dlq-*` error headers; an order published afterwards still applies, so the partition is not blocked. **The design deviates from instruction 7 on evidence:** the engine quarantines and *returns* a `QUARANTINED` outcome rather than throwing, so Spring's `DeadLetterPublishingRecoverer` would never fire — the listener reads outcomes and publishes instead. **`DlqPartitioningIT` and `QuarantineIdempotencyIT` are not implemented** (deferred) | [docs/results/s04/s04-t03-error-policy.txt](results/s04/s04-t03-error-policy.txt) | 2026-09-16 |
| Transient → retry → pause → resume | `TransientRetryIT`, `PauseResumeIT`, `UnclassifiedErrorIT` | **Partial.** `PausePolicyTest` **passed** (5 tests, unit): an exhausted transient failure pauses, seeks back to the batch's first offset and raises the paused gauge; an unclassified failure pauses rather than dead-lettering; an already-paused container is not paused twice; the probe resumes only once `SELECT 1` answers; a running container is never probed. Transient retry itself is already proven at engine level by S02's `ApplyLockTimeoutRetryIT`. **A container-level `PauseResumeIT` under a real database fault is NOT written** — a second Spring context joins the same `ledger-apply` group, splitting partitions between two consumers so the assertion would race. Recorded as deferred, not claimed. **Found a real defect:** the paused gauge was set inside the do-not-pause-twice guard, so a container already paused for another reason would have failed with no alert signal at all | [docs/results/s04/s04-t03-error-policy.txt](results/s04/s04-t03-error-policy.txt) | 2026-09-16 |
| Trace propagation across the pipeline (M12(a)) | Live stack, spans read from Tempo | **Connected (links) after CR-S04-01 — re-verified on a rebuilt stack.** The `payments.money-orders.v1 publish` span is now a **child inside the request trace** (`5a3c0d40536213f7c7171a3da765fb7e`), and the ledger's `process` span (`dff706fd0e8a31a0e01aa35543cda6cb`) **links into that same trace**. **First run found a real defect** and it is kept in the evidence: three disconnected traces, with the publish an orphan root (`6d95da62c5376119042c545083a5f9e4`) that no request owned. **Not claimed as parent-child end to end:** batch consumption links at the consumer, which S07-T01 must follow | [docs/results/s04-trace-propagation.md](results/s04-trace-propagation.md) | 2026-09-16 |
| Freshness contract | `FreshnessCalculatorTest`, `FreshnessIT`, `FreshnessContractTest` | **Passed** (7 + 3 + 1 tests). Unit: uncommitted partitions count from earliest not zero, zero lag is not listed as lagging, per-partition lag is reported so one stuck partition cannot hide in a total, an offset below the log start is an **error** rather than a small lag, one bad partition fails the whole measurement, a future timestamp clamps to zero. Integration: a caught-up ledger reports zero lag and zero age with every partition present. **Fail-closed proven through HTTP** — the contract test's context has no broker, so the endpoint takes its error path and the response still validates against the `Freshness` schema with the numbers null and a reason given. **Not covered:** the "records produced while the listener is stopped" case (it shares a cached context with the other pipeline tests) and the live partition-count-mismatch branch | [docs/results/s04/s04-t04-freshness.txt](results/s04/s04-t04-freshness.txt) | 2026-09-16 |
| M5 (a)–(d) end to end | `PipelineE2ETest` Cases A–C | **Not delivered as specified (D04-6).** Case A/B substance is covered at listener level by `LedgerListenerIT` and `DuplicateDeliveryIT`, and the API-to-relay half by S03's outbox evidence, but **no `PipelineE2ETest` exists** and Cases A–C are **not claimed to pass**. **Case C is Blocked on the D04-8 crash seam** | — | — |
| M4 (c) downstream half | `PipelineE2ETest` Case D | **Blocked.** Requires an injection point in `libs/outbox` to halt order-service between broker confirmation and the outbox rows being marked published. **No such injection point exists in D03-5**, and the task forbids forking the library to add one, so this is raised as **CR-S04-02** rather than worked around. Not claimed | — | — |
| M6 (c) audit walk | `PipelineE2ETest` Case E | **Passed, executed against the live Compose stack** (not as an automated suite — see D04-6). Balance → changelog → order in **3 calls**, within the M6(c) bound, using only public read APIs: `driver:D7` shows `payable −2400`; the changelog row gives `seq 1, delta −2400`, the order id and `source = trip-simulator / e2e-trip_e2e_1789617901`; fetching that order returns the same id and the **same idempotency key**, asserted equal. The order flowed API → outbox → relay → Kafka → apply, draining in 1 s and applying in 2 s | [docs/results/s04-trace-propagation.md](results/s04-trace-propagation.md) (same stack session) | 2026-09-16 |
| Mapper consumer poison policy | `MapperPoisonIT` | **Deferred, not run.** D04-4's classification and DLQ naming are not yet applied to order-service's payment-event consumer; that consumer quarantines in its own table (S03-T07) but still does not dead-letter, because a producer in its package would trip the M4(b) publish-path rule. Not claimed | — | — |
| Manual re-publish runbook step | Runbook dry run (T03) | **Partial.** The procedure is written — [docs/runbook.md](runbook.md), **created by this task**, since the step assumed the file existed and it did not: find the unresolved row, read its `error_code`, recover the original bytes from the quarantine row or the DLQ headers, re-publish under the original order-group key, confirm through `applied_orders`, then resolve with an `UPDATE` (never a `DELETE`, which the runtime role lacks). **The dry run was not executed**, so the steps are unverified in practice | [docs/runbook.md](runbook.md) | 2026-09-16 |
...
| S04-T02 | Done | `services/ledger-service/src/main/java/dev/zerosum/ledger/kafka/MoneyOrderListener.java`, `application.yml` (`ledger.consumer.enabled`), `…/api/LedgerApiExceptionHandler.java` (advice rescoped by package), `…/test/java/dev/zerosum/ledger/kafka/{LedgerPipelineTestBase,AckOrderingTest,LedgerListenerIT,DuplicateDeliveryIT,PartitionKeyIT}.java`, and `ledger.consumer.enabled=false` added to the four broker-less API tests | [docs/results/s04/s04-t02-listener.txt](results/s04/s04-t02-listener.txt): 8 new tests; 113 ledger tests green | **Deferred:** `GracefulShutdownIT`, the D04-8 seam, listener concurrency tuning, and counting a post-commit offset-commit failure. Three defects in my own tests are recorded in the evidence note |
| S04-T03 | Done | `services/ledger-service/src/main/java/dev/zerosum/ledger/kafka/{DlqPublisher,PauseOnFailureErrorHandler,ListenerResumeProbe,LedgerKafkaConfig}.java`, `MoneyOrderListener.java` (dead-letters quarantined outcomes before acknowledging), `application.yml` (`ledger.consumer.resume-probe-interval`), `…/test/java/dev/zerosum/ledger/kafka/{PausePolicyTest,PoisonRecordIT}.java` and a new ordering case in `AckOrderingTest`, plus [docs/runbook.md](runbook.md) (created here) | [docs/results/s04/s04-t03-error-policy.txt](results/s04/s04-t03-error-policy.txt): 11 tests across the three classes; **121 ledger tests green** | **Deviates from instruction 7 on evidence:** the engine returns a `QUARANTINED` outcome instead of throwing, so `DeadLetterPublishingRecoverer` would never have fired — the listener publishes from the outcomes instead. **Deferred:** a real-database-fault `PauseResumeIT` (a second context splits the consumer group), `DlqPartitioningIT`, `QuarantineIdempotencyIT`, `MapperPoisonIT`, probe-interval backoff, and the runbook dry run. Two defects of mine fixed: the paused gauge sat inside the do-not-pause-twice guard, and `TopicProvisioningIT` was timing-flaky |
| S04-T04 | Done | `services/ledger-service/src/main/java/dev/zerosum/ledger/kafka/{FreshnessCalculator,FreshnessView}.java`, `…/api/FreshnessController.java`, `…/kafka/LedgerKafkaConfig.java` (`freshnessAdminClient` bean), `openapi/ledger-service.yaml` (`/v1/freshness` + `Freshness` schema), `…/test/java/dev/zerosum/ledger/kafka/{FreshnessCalculatorTest,FreshnessIT}.java` and a fail-closed case in `LedgerOpenApiContractIT` | [docs/results/s04/s04-t04-freshness.txt](results/s04/s04-t04-freshness.txt): 11 tests; **132 ledger tests green** | The lag arithmetic was extracted into a pure function because the task's five unit cases were otherwise unreachable behind live admin calls. One self-inflicted regression: a bean named `kafkaAdmin` collided with Boot's own and broke all 36 Spring-context tests until renamed |
| S04-T05 | **Partial** | Case E executed against the live stack; no `PipelineE2ETest` source set added | [docs/results/s04-trace-propagation.md](results/s04-trace-propagation.md) | Cases A/B cited from listener-level evidence, **Case C Blocked** on D04-8, **Case D Blocked** on the absent D03-5 injection point (CR-S04-02). The container-image harness is deferred (D04-6) |
| S04-T06 | Done | [docs/results/s04-trace-propagation.md](results/s04-trace-propagation.md), `libs/outbox/.../OutboxRelay.java` (context restoration), `.../RelayTraceContextTest.java` | Trace IDs from both the failing and the fixed run | Found M12(a) broken at the outbox hop, **fixed it under CR-S04-01 and re-verified live**. Result is **connected (links)**, not parent-child — the consumer hop links, which is handed to S07-T01 |
| S04-T07 | **Deferred** | — | — | Relay-lag measurement not run; see D04-7. No publish-lag percentiles exist for this build, so the SP2 question is **unanswered rather than answered favourably**. Handed to S07-T05 |
| S04-C01 | **Not triggered** | — | — | Triggering requires evidence that the SP2 criterion was missed at the SP2 arrival rate. T07 was not run, so no such evidence exists — the spike is not triggered, and the relay is **not** thereby shown to be fast enough (D04-7) |
...
>
> - **The trace is connected, but by links rather than parent-child.** The outbox-hop defect CR-S04-01 found is
>   fixed and re-verified: the publish span is a child of the request trace. The consumer hop still **links**, which
>   is normal for batch consumption — **S07-T01 must follow links** rather than assume one parent-child trace.
...
<a id="s04-change-requests"></a>
### Change requests raised by S04

| ID | To | Against | Request | Status |
|---|---|---|---|---|
| **CR-S04-01** | S03 | D03-5 (`libs/outbox` relay) | **The relay must restore the stored trace context, not merely copy the header.** `OutboxWriter` writes `traceparent` into the row and `OutboxRelay.send()` copies stored headers onto the `ProducerRecord`, but the OTel agent builds the publish span from the relay thread's context — which is empty — and overwrites the copied header. The result is an orphan publish trace, so the request's trace does not reach Kafka and M12(a) is unmet. The fix belongs in `libs/outbox`: parse the stored `traceparent` and make it the active context (or an explicit parent) around the send. Evidence: [docs/results/s04-trace-propagation.md](results/s04-trace-propagation.md) | **Accepted and implemented 2026-09-16** — `OutboxRelay` now restores the stored context with `W3CTraceContextPropagator` around each send, covered by `RelayTraceContextTest` and **re-verified on the live stack**: the publish span is a child of the request trace |
| **CR-S04-02** | S03 | D03-5 (`libs/outbox` relay) | **An injection point is needed to halt a producing service between broker confirmation and marking outbox rows published**, so S04-T05 Case D (M4(c) downstream half) can be written. S04 did **not** add one: the task explicitly forbids forking `libs/outbox`, and a failure switch does not belong in shipped code without the owning step's decision. Case D stays Blocked until D03-5 provides a seam | **Raised 2026-09-16** |
