...
**34 MET · 8 PARTIAL · 1 NOT MET · 1 UNKNOWN**, of 44 lettered sub-criteria.
...
| M11(c) Reconciliation under chaos | MET | [m11c-recon-under-chaos.md](results/s06/m11c-recon-under-chaos.md): 5 seeded runs × 2 settlement cycles under A0 chaos (FakeCard lost responses 20 %, HTTP 500 5 %, webhooks duplicated/reordered/dropped, report lines missing/off-by-one/duplicated at 5 % each, `kill -9` of instrument-service and order-service every cycle). **405 of 405 breaks explained by the injected fault log, 0 unexplained, 0 undetected**; the final verifier (clean tree) passed I1, I6, I6b, I7, I12: 6,010 orders all applied, 3,000 attempts ↔ 3,000 provider charges, 0 duplicates. Day close simulated by backdating; 600 trips per run, not the master's 10,000 |
...
| M13(b) Ablations A1–A4 | PARTIAL | [ablation-results.md](results/s08/ablation-results.md): 60 evidence runs with chaos-profile switches (refused without `ZS_ALLOW_CHAOS=true`) and a run plan pre-registered before evidence. **A2, A3, A4 and B0 valid**: without the outbox, orders were lost (2 per `kill -9`, 21–32 per hard broker outage); a fresh provider key gave 26–28 duplicate charges per 200 trips; without the zero-sum checks, 55–70 unbalanced orders reached the ledger; everything off gave two or more violation classes in 5 of 6 runs. A5 showed no violation, as predicted. **A1 not valid**: it inflated balances in 3 of 3 ledger crashes (up to $28,398 of drift), but no broker fault (0 of 6) produced a redelivery, and the pre-registered rule counts those cells. **The full design made 0 violations in 21 of 21 control runs** |
...
| M14(a) Fresh clone → W1 in ≤ 10 min | MET | **Cold: 4.83 min** from `git clone` to a completed W1 with every project image, the BuildKit cache and the Gradle home emptied first (1.4 GB Gradle + ≈ 5.9 GB images downloaded at ≈ 143 Mbit/s); warm: 41 s ([fresh-clone.md](results/m14/fresh-clone.md)). Host JDK, git and Docker Desktop were installed, as the README's prerequisites |
...
|---|---|---|
| Unit | 372 (0 failed, 1 skipped) | No containers |
| Integration | 313 (0 failed, 0 skipped) | Real PostgreSQL and Kafka via Testcontainers |
...
Unit and integration counts are from `./gradlew test integrationTest --rerun-tasks` on 2026-09-19 (09:22–09:37 UTC,
commit `f348799`), with no Compose stack running, read out of
...
was taken on. Since the last count: 12 unit tests for the chaos-switch guard (every switch ships off and is refused outside the
`chaos` profile), 9 integration tests for the verifier's cross-store checks, and the settlement-report race's
regression test.
