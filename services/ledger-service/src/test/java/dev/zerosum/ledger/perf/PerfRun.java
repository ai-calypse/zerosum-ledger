 ...
 *
 * <p>{@code windowStartUtc} and {@code windowEndUtc} bound the measured window on the host clock. They exist so each
 * window can be checked against anything else that ran on the same machine: a laptop Docker VM is shared, and a number
 * whose window overlapped other load is a lower bound rather than a clean figure.
 */
record PerfRun(String measurement, int writers, int batchSize, int entityCount, int repetition, int windowSeconds,
        String windowStartUtc, String windowEndUtc, long appliedOrders, double ordersPerSecond, long batchP50Micros,
        long batchP95Micros, long lockWaitP50Micros, long lockWaitP95Micros, long batches, long duplicates,
        long quarantined, long deadlockRetries, long lockTimeoutRetries, long connectionRetries, long retriesExhausted,
    ...
    PerfRun withI5(long violations) {
        return new PerfRun(measurement, writers, batchSize, entityCount, repetition, windowSeconds, windowStartUtc,
                windowEndUtc, appliedOrders, ordersPerSecond, batchP50Micros, batchP95Micros, lockWaitP50Micros,
                lockWaitP95Micros, batches, duplicates, quarantined, deadlockRetries, lockTimeoutRetries,
                connectionRetries, retriesExhausted, i2Violations, i3Violations, i4Violations, violations);
    }

    /** Apply cost per order at the median batch, which is the batch duration divided over the orders it carried. */
    double microsPerOrderAtP50() {
        return batchSize == 0 ? 0 : (double) batchP50Micros / batchSize;
    }

    String summary() {
        return String.format("%s writers=%d batch=%d entities=%d rep=%d window=%s..%s applied=%d ordersPerSecond=%.1f "
                ...
                measurement, writers, batchSize, entityCount, repetition, windowStartUtc, windowEndUtc, appliedOrders,
    ...
    String toJson() {
        return String.format("{\"measurement\": \"%s\", \"writers\": %d, \"batch_size\": %d, \"entity_count\": %d, "
                        + "\"repetition\": %d, \"window_seconds\": %d, \"window_start_utc\": \"%s\", "
                        + "\"window_end_utc\": \"%s\", \"applied_orders\": %d, "
                ...
                measurement, writers, batchSize, entityCount, repetition, windowSeconds, windowStartUtc, windowEndUtc,
                appliedOrders, ordersPerSecond, batchP50Micros, batchP95Micros, microsPerOrderAtP50(),
                lockWaitP50Micros, lockWaitP95Micros, batches, duplicates, quarantined, deadlockRetries,
                lockTimeoutRetries, connectionRetries, retriesExhausted, i2Violations, i3Violations, i4Violations,
                i5Violations, valid());
    }
}
