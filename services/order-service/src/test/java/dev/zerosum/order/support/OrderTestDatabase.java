package dev.zerosum.order.support;
 ...
 * image; D00-3, D00-4), with the orders database migrated by Flyway as {@code orders_owner}.
 *
 * <p>This mirrors ledger-service's {@code LedgerTestDatabase}. S02 recorded the rule for that duplication: a third
 * service needing it triggers a change request to D00-2 to extract a shared fixture. order-service is the second, so
 * the copy stands; S05's instrument-service is expected to be the third and to raise that request.
 */
public final class OrderTestDatabase implements AutoCloseable {

    public static final Path ROOT = Path.of(System.getProperty("zs.rootDir"));
    public static final String OWNER = "orders_owner";
    public static final String APP = "orders_app";
    ...
    /** Starts the container and migrates the orders database to the latest version. */
    public static OrderTestDatabase start() {
        OrderTestDatabase db = new OrderTestDatabase();
        db.container.start();
        db.flyway().migrate();
        return db;
    }
    ...
    /** A connection to the orders database as one of the D00-4 roles. */
    ...
    /** A {@link javax.sql.DataSource} for one of the D00-4 roles. */
    ...
    /** A pooled data source, for tests that need more connections than the writer count. */
    public com.zaxxer.hikari.HikariDataSource pooledDataSource(String role, int poolSize) {
        ...
        config.setPoolName("orders-" + role);
